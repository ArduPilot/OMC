...
public class CompoundElevationModel extends AbstractElevationModel
{
    ...
    {
        return getElevations(sector, latlons, targetResolution, buffer, null);
    }

    public double getElevations(Sector sector, List<? extends LatLon> latlons, double targetResolution, double[] buffer, ElevationModelRerence bestModel)
    {
        ...
        double[] res = this.doGetElevations(sector, latlons, targetResolutions, buffer, false, bestModel);
        ...
        double[] buffer)
    {
        ...
        double[] res = this.doGetElevations(sector, latlons, targetResolutions, buffer, false,null);
        double bestRes=res[0];
        for (int i =1; i < res.length; i++){
        	if (res[i]<bestRes) bestRes = res[i];
        }
    ...
    public double[] getElevations(Sector sector, List<? extends LatLon> latLons, double[] targetResolutions,
        double[] elevations)
    {
        return this.doGetElevations(sector, latLons, targetResolutions, elevations, false,null);
    }
    ...
    protected double[] doGetElevations(Sector sector, List<? extends LatLon> latlons, double[] targetResolution,
                                       double[] buffer, boolean mapMissingData, ElevationModelRerence bestModel)
    {
        if (sector == null)
        {
            String msg = Logging.getMessage("nullValue.SectorIsNull");
        ...
        for (int i = 0; i < this.elevationModels.size(); i++)
        {
            ...
            if (Math.abs(r) < resolutionAchieved[i] || resolutionAchieved[i] == 0) {
                resolutionAchieved[i] = Math.abs(r);
                if (bestModel != null && r > 0) {
                    bestModel.elevationModel = em;
                }
            }
        }
    ...
    public static class ElevationModelRerence {
