...
public abstract class URLRetriever extends WWObjectImpl implements Retriever {
    ...
    public static URLRetriever createRetriever(URL url, RetrievalPostProcessor postProcessor) {
        ...
        else if ("jar".equalsIgnoreCase(protocol)) return new JarRetriever(url, postProcessor);
        else return null;
    }
    ...
    protected URLConnection openConnection() throws IOException {
        try {
            // We always use the default proxy (i.e. ProxySelector.getDefault()
            this.connection = this.url.openConnection();
            /*
            Proxy proxy = WWIO.configureProxy(this.url);
            if (proxy != null) this.connection = this.url.openConnection(proxy);
            else this.connection = this.url.openConnection(Proxy.NO_PROXY);
            */
        } catch (java.io.IOException e) {
            Logging.logger()
                .log(Level.SEVERE, Logging.getMessage("URLRetriever.ErrorOpeningConnection", this.url.toString()), e);
            throw e;
        }
    ...
    protected void configureSSLContext(HttpsURLConnection connection) {
        ...
        if (sslContext != null) connection.setSSLSocketFactory(sslContext.getSocketFactory());
    }
    ...
    protected void end() throws Exception {
        try {
            ...
            this.setState(RETRIEVER_STATE_ERROR);
            Logging.logger()
                .log(Level.SEVERE, Logging.getMessage("Retriever.ErrorPostProcessing", this.url.toString()), e);
            throw e;
        }
    }
    ...
    protected ByteBuffer doRead(URLConnection connection) throws Exception {
        ...
        try {
            ...
            else buffer = this.readNonSpecificStream(inputStream, connection);
        } finally {
     ...
     *
     * <p>If both the Expires and Date headers are present then the expiration time is calculated as current time +
    ...
    protected long getExpiration(URLConnection connection) {
        ...
        if (cacheControl != null) {
            ...
            if (matcher.find()) {
                ...
                if (maxAgeSec != null) return maxAgeSec * 1000 + System.currentTimeMillis();
            }
        }
        ...
        if (date > 0 && expiration > date) return System.currentTimeMillis() + (expiration - date);
