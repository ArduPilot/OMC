...
//            this.expirySupport.restart(dc);
    ...
    public void drawProfile(DrawContext dc)
    {
        ...
        try
        {
            ...
            Dimension drawSize = isMinimized ? new Dimension(MINIMIZED_SIZE, MINIMIZED_SIZE) :
                isMaximized ? new Dimension(viewport.width - this.borderWidth * 3,
            ...
            if (!dc.isPickingMode())
            {
                ...
                if (!isMinimized)
                {
                    ...
                    //fontSize * 2 - shifts the label 2 lines below to leave the space for the "pickedSample", takes into account scaling factor
                    drawLabel(dc, label, locationSW.add3(new Vec4(0, -defaultFont.getSize() * (this.pickedSample != -1 ? 2 : 1), 0)), -1); // left aligned
                    if (this.pickedSample != -1)
                    {
                        ...
                        //aligned left to have unified look on different scales
                        drawLabel(dc, label, locationSW.add3(new Vec4(0, -defaultFont.getSize(), 0)), -1); // aligned also left
                    }
                }
            }
            else
    ...
    public void selected(SelectEvent event)
    {
        ...
        {
            // in javaFX port this events might be empty
