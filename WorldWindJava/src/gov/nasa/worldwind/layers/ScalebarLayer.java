...
public class ScalebarLayer extends AbstractLayer
{
    ...
    protected double displayScaleFactor = 1;
            ...
            if (orderedImage.pixelSize > 0)
            {
                ...
                if (!Double.isNaN(pot))
                {
                    ...
                    if (!dc.isPickingMode())
                    {
                        ...
                        //scaleFactor because height depends on font size and it depends on display scale
                        drawLabel(dc, label,
                            locationSW.add3(
                                new Vec4(divWidth * scale / 2 + (width - divWidth) / 2, height * displayScaleFactor, 0)));
                    }
                    else
                    {
                        // Picking
    ...
    public void setDisplayScaleFactor(double displayScaleFactor) {
