...
public class KMLModelPlacemarkImpl extends WWObjectImpl implements KMLRenderable, ColladaResourceResolver
{
    ...
    protected void configureColladaRoot(ColladaRoot root)
    {
        ...
        if (root.getAltitudeMode()==WorldWind.ABSOLUTE){
            //in intel mission control, absolute altitudes are relative to WGS84, but KML expects them relative to EGM, so lets transform
            if (egm96 ==null) {
                try{
            ...
            refPosition = new Position(refPosition, refPosition.elevation+ egm96.getOffset(refPosition.latitude,refPosition.longitude));
        }
