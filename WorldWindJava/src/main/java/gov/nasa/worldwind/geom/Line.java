...
//    }
    ...
    public final double distanceTo(Vec4 p) {
        ...
        // Vec4 w = p.subtract3(this.origin);
        tmp[0] = p.getX() - getOrigin().getX();
        tmp[1] = p.getY() - getOrigin().getY();
        tmp[2] = p.getZ() - getOrigin().getZ();

        // double c1 = w.dot3(this.direction);
        final double c1 = tmp[0] * getDirection().getX() + tmp[1] * getDirection().getY() + tmp[2] * getDirection().getZ();

        // double c2 = this.direction.dot3(this.direction);
        final double c2 =
            getDirection().getX() * getDirection().getX()
                + getDirection().getY() * getDirection().getY()
                + getDirection().getZ() * getDirection().getZ();

        final double f = c1 / c2;

        // return this.origin.add3(this.direction.multiply3(c1 / c2)); = vec4
        tmp[0] = f * getDirection().getX() + getOrigin().getX();
        tmp[1] = f * getDirection().getY() + getOrigin().getY();
        tmp[2] = f * getDirection().getZ() + getOrigin().getZ();

        double res = 0;
        double s = p.getX() - tmp[0];
        res += s * s;
        s = p.getY() - tmp[1];
        res += s * s;
        s = p.getZ() - tmp[2];
    ...
    public static List<Vec4> segmentIntersectionBall(Vec4 p0, Vec4 p1, Vec4 center, double radius) {
        List<Vec4> res = new LinkedList<>();
        if (radius < 0) {
            return res;
        }

        Vec4 deltaSegment = p1.subtract3(p0);
        Vec4 offsetSphere = p0.subtract3(center);

        double a = deltaSegment.dotSelf3();
        double b = 2 * offsetSphere.dot3(deltaSegment);
        double c = offsetSphere.dotSelf3() - radius * radius;
        ...
        if (discriminant >= 0) {
            ...
            if (t1 >= 0 && t1 <= 1) {
                res.add(p0.add3(deltaSegment.multiply3(t1)));
            }

            if (t2 >= 0 && t2 <= 1 && t1 != t2) {
                res.add(p0.add3(deltaSegment.multiply3(t2)));
            }
        }
