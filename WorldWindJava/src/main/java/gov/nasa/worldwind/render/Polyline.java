    ...
    MeasurableLength, ExtentHolder, PreRenderable, Highlightable, Draggable
{
    ...
    {
        this.reset();
        if (inPositions instanceof Collection) {
            //this is often a collection (well, as of 2018/5/29 all calls were collections
            //the default size of the ArrayList is 10, however, we usually call this method with sizes of either
            //2,3 or for large flightplans with 1k+ objects. In the first case, we waste memory, in the second case, we
            //have to re-size the arraylist each time its capacity limit is hit ... this happens a lot of time when adding
            //large objects which causes a lot of copying of elements, thereby causing unnecessary GC pressure
            this.positions = new ArrayList<Position>(((Collection)inPositions).size());
        } else {
