    ...
    protected class ScheduledTaskExecutor extends ScheduledThreadPoolExecutor
    {
        protected ScheduledTaskExecutor(int poolSize)
        {
            super(poolSize,
                new ThreadFactory()
                {
                    private final int poolId = poolCount.incrementAndGet();
                    private final AtomicInteger threadCount = new AtomicInteger(0);
                    public Thread newThread(Runnable runnable)
                    {
                        Thread thread = new Thread(runnable, "WorldWind-basic-scheduled-task-service-" + poolId + "-worker-" + threadCount.incrementAndGet());
