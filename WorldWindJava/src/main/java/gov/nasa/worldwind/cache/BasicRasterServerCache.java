...
public class BasicRasterServerCache extends OldBasicMemoryCache
{
    ...
    public BasicRasterServerCache()
    {
        ...
        int threadId = threadCount.incrementAndGet();
        new Thread(new MemoryMonitorThread(), "WorldWind-Memory-Monitor-"+threadId).start();
        new Thread(new CachePrunerThread(), "WorldWind-Cache-Pruner-"+threadId).start();
    }
