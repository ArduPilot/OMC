...
//                this.loadConfigProperties(doc);
    ...
    private static GLProfile maxCompatibleGLProfile;
    /**
    ...
    {
        if (maxCompatibleGLProfile == null){
            maxCompatibleGLProfile = GLProfile.getMaxFixedFunc(true); // Favor a hardware rasterizer.
        }
        return maxCompatibleGLProfile;
    }


    private static GLCapabilities requiredGLCapabilities;
    ...
    {
        if (requiredGLCapabilities == null) {
            requiredGLCapabilities = new GLCapabilities(getMaxCompatibleGLProfile());

            requiredGLCapabilities.setAlphaBits(8);
            requiredGLCapabilities.setRedBits(8);
            requiredGLCapabilities.setGreenBits(8);
            requiredGLCapabilities.setBlueBits(8);
            requiredGLCapabilities.setDepthBits(24);
            requiredGLCapabilities.setDoubleBuffered(true);
            ...
            if ("device".equals(stereo))
                requiredGLCapabilities.setStereo(true);
        }
        return requiredGLCapabilities;
    }
