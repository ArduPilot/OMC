...
public abstract class TiledRasterProducer extends AbstractDataStoreProducer
{
    ...
    protected void installTileRaster(Tile tile, DataRaster tileRaster, AVList params) throws java.io.IOException
    {
        ...
        if (writer instanceof DataRasterWriter)
        {
            try
            {
                ...
                diskUsage += installLocation.length();
            }
    ...
    protected void installConfigFile(AVList params) throws Exception
    {
        ...
        WWXML.appendLong(configDoc.getDocumentElement(), "DiskUsage", getDiskUsage());
        Object sourceFileDate = params.getValue(AVKey.SOURCE_FILE_LAST_MODIFIED);
        if (sourceFileDate instanceof Long){
            WWXML.appendLong(configDoc.getDocumentElement(), "SourceFileDate", (Long)sourceFileDate);
        }
