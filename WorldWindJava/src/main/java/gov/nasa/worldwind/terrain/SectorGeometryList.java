...
public class SectorGeometryList extends ArrayList<SectorGeometry> {
    ...
    protected SectorTree sectorTree;
     ...
     *
     * <p>Note: Prior to calling this method, {@link #beginRendering(gov.nasa.worldwind.render.DrawContext)} must be
     * called.
     *
     * @param dc the current draw context.
    ...
    public Vec4 getSurfacePoint(Angle latitude, Angle longitude, double metersOffset) {
        ...
        SectorTree n = sectorTree.contains(latitude, longitude, false);

        if (n != null) {
            SectorGeometry s = n.getSectorGeometry();
            if (s != null) {
                Vec4 point = s.getSurfacePoint(latitude, longitude, metersOffset);
                if (point != null) {
                    return point;
                }
            }
        ...
        for (SectorGeometry sg : this) {
            if (sg.getSector().contains(latitude, longitude)) {
                ...
                if (point != null) {
                    Logging.logger().warning("I should not be called anymore, see IMC-2239");
                    return point;
                }
            }
        }
    ...
    private void initSectorTree() {
        sectorTree = new SectorTree(getSector());
        for (int i = 0; i < size(); i++) {
            SectorGeometry geom = get(i);
            if (geom != null) {
                Sector s = geom.getSector();
                sectorTree.addChild(new SectorTree(s, geom));
            }
        }
    ...
    public Intersection[] intersect(Line line) {
        ...
        for (SectorGeometry sg : sglist) {
            if (sg.getExtent().intersects(line))
                if ((hits = sg.intersect(line)) != null) list.addAll(Arrays.asList(hits));
        }
     ...
     *
     * <p>The returned array of <code>Intersection</code> describes a list of individual segments - two <code>
     * Intersection</code> for each, corresponding to each geometry triangle that intersects the given elevation.
     *
     * <p>Note that the provided bounding <code>Sector</code> only serves as a 'hint' to avoid processing unnecessary
    ...
    public Intersection[] intersect(double elevation, Sector sector) {
        ...
        for (SectorGeometry sg : sglist) {
            if (sector.intersects(sg.getSector()))
                if ((hits = sg.intersect(elevation)) != null) list.addAll(Arrays.asList(hits));
        }
