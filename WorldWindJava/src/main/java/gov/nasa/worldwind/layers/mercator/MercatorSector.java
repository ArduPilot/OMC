...
public class MercatorSector extends Sector
{
    ...
    public double getDeltaLatPercent() { return maxLatPercent - minLatPercent;}


    @Override
    public String toString() {
        return super.toString() + " lat%: (" + minLatPercent + ", " + maxLatPercent + ")";
    }

    public static int computeRow(int level, Angle latitude){
        double delta = Math.pow(2, 1-level);
        int row = (int) ((gudermannianInverse(latitude) + 1) / delta);

        //suspicious behaviour, maybe a rounding issue in the line above - sometimes row has MAX_VALUE + 1
