...
public class QuantityFormat {
    ...
    private FillMode fillMode = FillMode.NO_FILL;
    ...
    public void setSystemOfMeasurement(SystemOfMeasurement systemOfMeasurement) {
        if (systemOfMeasurement == null) {
            throw new IllegalArgumentException("systemOfMeasurement");
        }
    ...
    public void setAngleStyle(AngleStyle angleStyle) {
        if (angleStyle == null) {
            throw new IllegalArgumentException("angleStyle");
        }
    ...
    public void setTimeStyle(TimeStyle timeStyle) {
        if (timeStyle == null) {
            throw new IllegalArgumentException("timeStyle");
        }
    ...
    public void setFillMode(FillMode fillMode) {
        if (fillMode == null) {
            throw new IllegalArgumentException("fillMode");
        }
    ...
    private String formatTime(VariantQuantity time) {
        if (getTimeStyle() == TimeStyle.HOUR_MINUTE_SECOND) {
            ...
            stringBuilder.append(minutesInt);
            stringBuilder.append(':');
            ...
            stringBuilder.append(secondsInt);
    ...
    private <Q extends Quantity<Q>> String formatNumber(Number number, Unit<?> unit) {
        ...
        if (fillMode == FillMode.ZERO_FILL) {
            decimalFormat.setMinimumFractionDigits(remainingFracDigits);
        } else {
        ...
        if (formattedNumber.charAt(0) == '-' && Math.abs(number.doubleValue()) < Math.pow(1, -significantDigits)) {
            formattedNumber = formattedNumber.substring(1);
        }

        return formattedNumber + formatSymbol(unit.getDisplaySymbol());
    }
