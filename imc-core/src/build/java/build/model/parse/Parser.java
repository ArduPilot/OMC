...
public class Parser {
    ...
    private static final List<String> KNOWN_TYPES = List.of("String", "OffsetDateTime");
    ...
    public ClassNode parse(String text) throws IOException {
        try (BufferedReader reader = new BufferedReader(new StringReader(text))) {
            ...
            List<TypeNode> implementedInterfaces = new ArrayList<>();
            boolean abstractClass = false;
            boolean inheritsBaseModel = false;
            ...
            while ((line = reader.readLine()) != null) {
                ...
                if (!tokenizer.get().equals(Tokenizer.NO_TOKEN) && !tokenizer.get().startsWith("#")) {
                    if (tokenizer.get().equals("abstract") && tokenizer.peek().equals(Tokenizer.NO_TOKEN)) {
                        abstractClass = true;
                    } else if (tokenizer.get().equals("inherits") && !tokenizer.peek().equals(":")) {
                        tokenizer.consume();

                        if (tokenizer.get().equals("abstract")) {
                            tokenizer.consume();
                            inheritsBaseModel = true;
                        }

                        inheritedClass = tokenizer.getIdentifier(lineNumber);
                    } else if (tokenizer.get().equals("implements") && !tokenizer.peek().equals(":")) {
                        tokenizer.consume();

                        while (!tokenizer.get().equals(Tokenizer.NO_TOKEN)) {
                            implementedInterfaces.add(parseType(tokenizer, lineNumber));
                            if (tokenizer.get().equals(",")) {
                                tokenizer.consume();
                            }
                        }
                    } else if (tokenizer.get().equals("immutable")) {
                        tokenizer.consume();
                        isImmutable = true;
                    }
                    else {
            ...
            return new ClassNode(
                className, inheritedClass, abstractClass, inheritsBaseModel, isImmutable, implementedInterfaces, fields);
        }
    }
    ...
    private TypeNode parseType(Tokenizer tokenizer, int lineNumber) {
        ...
        if (innerType != null) {
            ...
            default:
                return new UnknownGenericTypeNode(outerType, innerType, attributes);
            }
        }

        if (KNOWN_TYPES.contains(outerType)) {
            return new KnownTypeNode(outerType, attributes);
        } else if (generatedClassNames.contains(outerType)) {
            return new GeneratedTypeNode(outerType, attributes);
        }
