...
import org.droidplanner.services.android.impl.core.mission.commands.ImageStartCaptureImpl;
...
import org.droidplanner.services.android.impl.core.mission.waypoints.MountControlImpl;
...
public class ProxyUtils {
    ...
    public static MissionItemImpl getMissionItemImpl(MissionImpl missionImpl, MissionItem proxyItem) {
        ...
        switch (proxyItem.getType()) {
            ...
            case START_IMAGE_CAPTURE: {
                StartImageCapture proxy = (StartImageCapture) proxyItem;

                ImageStartCaptureImpl temp = new ImageStartCaptureImpl(missionImpl, (proxy.getNumberOfImagesToCapture()));

                missionItemImpl = temp;
                break;
            }
            case MOUNT_CONTROL: {
                MountControl proxy = (MountControl) proxyItem;

                MountControlImpl temp = new MountControlImpl(missionImpl, (proxy.getCoordinate()));

                temp.setPitchAngle(proxy.getPitchAngle());
                temp.setYawAngle(proxy.getYawAngle());
                temp.setRollAngle(proxy.getRollAngle());
                missionItemImpl = temp;
                break;
            }
            case CHANGE_SPEED: {
