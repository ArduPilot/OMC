    ...
    public static void createMenuItems(
        ...
        for (MenuModel model : menuModel.getChildren()) {
            ...
            if (model.isGroup()) {
                    ...
                    .addListener(
                        (observable, oldValue, newValue) -> {
                            if (newValue != null) {
                                ...
                                MenuModel itemModel = (MenuModel)newValue.getUserData();
                                checkGroupModel.setCheckedItem(itemModel);
                            }
                        });
                ...
                final Command command = model.getCommand();

                MenuItem menuItem;
                if (model.isCheckable()) {
                    RadioMenuItem checkMenuItem = new RadioMenuItem();
                    checkMenuItem.setUserData(model);

                    if (toggleGroup != null) {
                        checkMenuItem.setToggleGroup(toggleGroup);
                        checkMenuItem.setSelected(model.isChecked());
                    } else {
                    ...
                    event -> {
                        if (command.isExecutable()) {
                            if (command instanceof ParameterizedCommand) {
                                Optional<Object> param = model.getCommandParameter();
                                if (param.isPresent()) {
                                    //noinspection unchecked
                                    ((ParameterizedCommand)command).execute(param.get());
                                } else {
                                    command.execute();
                                }
                            } else {
                        ...
                        Bindings.createObjectBinding(
                            () -> {
                                if (model.imageSourceProperty().get() != null) {
                                    ImageView imageView =
                                        new ImageView(model.imageSourceProperty().get().toExternalForm());
