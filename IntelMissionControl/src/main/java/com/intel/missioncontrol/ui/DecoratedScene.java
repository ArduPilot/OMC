...
public class DecoratedScene extends Scene {
    ...
    private static class WindowMetrics {
        ...
        WindowMetrics(double dpiFactor) {
            ...
            try {
                ...
                frameWidthScaled =
                    User32Ex.INSTANCE.GetSystemMetricsForDpi(User32.SM_CXFRAME, dpi)
                        + User32Ex.INSTANCE.GetSystemMetricsForDpi(User32.SM_CXPADDEDBORDER, dpi);
    ...
    private interface UxTheme extends StdCallLibrary, WinUser, WinNT {
        UxTheme INSTANCE = Native.loadLibrary("uxtheme", UxTheme.class, W32APIOptions.DEFAULT_OPTIONS);
    ...
    private interface Comctl32 extends StdCallLibrary, WinUser, WinNT {
        Comctl32 INSTANCE = Native.loadLibrary("comctl32", Comctl32.class, W32APIOptions.DEFAULT_OPTIONS);
        ...
        window.outputScaleXProperty()
            .addListener(
                ((observable, oldValue, newValue) -> {
                    dpiFactor = newValue.doubleValue();
                    windowMetrics = new WindowMetrics(newValue.doubleValue());
                }));
    ...
    private void updateShadowBorder(HWND hwnd) {
        ...
        if (this.windowRect != null
                && this.windowRect.top == windowRect.top
                && this.windowRect.right == windowRect.right
                && this.windowRect.bottom == windowRect.bottom
                && this.windowRect.left == windowRect.left) {
            return;
        }
