...
public class TitledPaneSkin extends LabeledSkinBase<TitledPane> {
    ...
    private static final Duration TRANSITION_DURATION = new Duration(200);
    private static final Interpolator EASE_OUT = Interpolator.SPLINE(0.1, 0.6, 0.5, 0.95);
    ...
    private static final boolean CACHE_ANIMATION = true;
    ...
    private static Field graphicField;
    private static Field graphicPropertyChangedListenerField;

    static {
        try {
            textField = LabeledSkinBase.class.getDeclaredField("text");
            textField.setAccessible(true);
            graphicField = LabeledSkinBase.class.getDeclaredField("graphic");
            graphicField.setAccessible(true);
            graphicPropertyChangedListenerField =
                LabeledSkinBase.class.getDeclaredField("graphicPropertyChangedListener");
            graphicPropertyChangedListenerField.setAccessible(true);
...
//        control.setInputMap(behavior.getInputMap());
    ...
    @Override protected void layoutChildren(final double x, double y,
        ...
        // content
        contentContainer.setOpacity(getTransition());
    ...
    private void doAnimationTransition() {
        ...
        if (getSkinnable().isExpanded()) {
            ...
            k2 = new KeyFrame(
                    ...
                    new KeyValue(transitionProperty(), 1, EASE_OUT)
            ...
            k2 = new KeyFrame(
                    ...
                    new KeyValue(transitionProperty(), 0, EASE_OUT)
            );
        }
    ...
    private Node getGraphic() {
        try {
            return (Node)graphicField.get(this);
        } catch (ReflectiveOperationException e) {
            return null;
        }
    }

    private Node setGraphic(Node graphic) {
        try {
            graphicField.set(this, graphic);
            return graphic;
        } catch (ReflectiveOperationException ignored) {
            return null;
        }
    ...
    private LabeledText getText() {
        try {
            return (LabeledText)textField.get(this);
        } catch (ReflectiveOperationException e) {
            return null;
        }
    }

    private LabeledText setText(LabeledText text) {
        try {
    ...
    private InvalidationListener getGraphicPropertyChangedListener() {
        try {
            return (InvalidationListener)graphicPropertyChangedListenerField.get(this);
        } catch (ReflectiveOperationException e) {
            return null;
        }
    }
    ...
    class TitleRegion extends StackPane {
        ...
        private void update() {
            ...
            Node graphic = getGraphic();
            InvalidationListener graphicPropertyChangedListener = getGraphicPropertyChangedListener();
            ...
            graphic = setGraphic(titledPane.getGraphic());
            ...
            if (isIgnoreGraphic()) {
                ...
                if (isIgnoreText()) {
                    ...
                    getChildren().addAll(graphic, getText());
                }
            }
        ...
        private double labelPrefWidth(double height) {
            ...
            final Labeled labeled = getSkinnable();
            final Font font = getText().getFont();
            ...
            double textWidth = emptyText ? 0 : com.sun.javafx.scene.control.skin.Utils.computeTextWidth(font, string, 0);
        ...
        private double labelPrefHeight(double width) {
            final Labeled labeled = getSkinnable();
            final LabeledText text = getText();
            ...
            if (!isIgnoreGraphic() &&
                ...
                width -= (getGraphic().prefWidth(-1) + gap);
            }
