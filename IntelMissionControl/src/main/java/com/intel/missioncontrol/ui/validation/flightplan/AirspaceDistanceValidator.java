...
public class AirspaceDistanceValidator extends OnFlightplanRecomputedValidator {
    ...
    protected boolean onInvalidated(FlightPlan flightplan) {
        ...
        IResolveAction resolveSafetyAltitude =
            new SimpleResolveAction(
                languageHelper.getString(className + ".resolve"),
                () -> {
                    flightplan.autoComputeSafetyHeightProperty().set(true);
                    double safetyAtitude = flightplan.safetyAltitudeProperty().get();
                });

        IResolveAction[] allResolveSafetyAltitudeActions = new IResolveAction[] {resolveSafetyAltitude};
        ...
        double safetyAtitude = flightplan.safetyAltitudeProperty().get();
        double safetyAltitudeMSL = safetyAtitude + flightplan.getLegacyFlightplan().getTakeofftAltWgs84WithElevation();

        if (safetyAltitudeMSL > settings.maxAltitudeAboveSeaLevelProperty().doubleValue()) {
            addWarning(
                languageHelper.getString(
                    className + ".tooHighSafetyAltitudeOverMSL",
                    quantityFormat.format(
                        Quantity.of(
                            safetyAltitudeMSL - settings.maxAltitudeAboveSeaLevelProperty().doubleValue(), Unit.METER),
                        UnitInfo.LOCALIZED_LENGTH)),
                ValidationMessageCategory.NORMAL,
                allResolveSafetyAltitudeActions);
        }

        if (safetyAtitude > settings.maxAltitudeAboveGroundProperty().doubleValue()) {
            addWarning(
                languageHelper.getString(
                    className + ".tooHighSafetyAltitudeOverGround",
                    quantityFormat.format(
                        Quantity.of(
                            safetyAtitude - settings.maxAltitudeAboveGroundProperty().doubleValue(), Unit.METER),
                        UnitInfo.LOCALIZED_LENGTH)),
                ValidationMessageCategory.NORMAL,
                allResolveSafetyAltitudeActions);
        }
