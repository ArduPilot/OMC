...
import com.intel.missioncontrol.common.DoubleHelper;

public class LongBaseVariantQuantity implements VariantQuantity {

    private final long baseValue;
    private final double value;
    private final Unit<?> unit;

    LongBaseVariantQuantity(long baseValue, Unit<?> unit) {
    ...
    public VariantQuantity add(VariantQuantity quantity) {
        ...
        if (unit == quantity.getUnit()) {
            return new LongBaseVariantQuantity(baseValue + quantity.getBaseValue().longValue(), unit);
        }

        return new LongBaseVariantQuantity(baseValue + quantity.convertTo(unit).getBaseValue().longValue(), unit);
    }

    @Override
    public VariantQuantity add(Number value) {
        return new LongBaseVariantQuantity(baseValue + (long)unit.convertToBase(value.doubleValue()), unit);
    }
    ...
    public VariantQuantity multiply(Number scalar) {
        return new LongBaseVariantQuantity((long)(baseValue * scalar.doubleValue()), unit);
    }
    ...
    public VariantQuantity negate() {
        return new LongBaseVariantQuantity(-baseValue, unit);
    }
