...
public class GeoTiffEntry {
    ...
    private void load() {
        if (getLoadingState() == LoadingState.NOT_STARTED) {
            geotiffHelper
                .importGeoTiffAsync(this)
                .whenFailed(this::errorLoading)
                .whenSucceeded(
                    (geoTiffEntry) -> {
                                    ...
                                    protected Void call() throws Exception {
                                        ...
                                        if (em != null) {
                                            ...
                                            PropertyHelper.setValueSafe(
                                                geoTiffEntry.elevationModelShiftType,
                                            ...
                                            Dispatcher.background()
                                                .runLater(em::autoAdjustShift, Duration.ofMillis(1000));
                                            Dispatcher.background()
                                                .runLater(em::autoAdjustShift, Duration.ofMillis(3000));
                                            Dispatcher.background()
                                                .runLater(
                                                    () -> {
                                                        em.autoAdjustShift();
                                                        PropertyHelper.setValueSafe(
                                                            geoTiffEntry.shift, Quantity.of(em.getShift(), Unit.METER));
                                                    },
                            ...
                            loadingTask.addEventHandler(
                                ...
                                event -> {
                                    if (event.getEventType() == WorkerStateEvent.WORKER_STATE_CANCELLED) {
                                        Dispatcher.platform().run(() -> geotiffHelper.dropGeotiffImport(geoTiffEntry));
                                    } else if (event.getEventType() == WorkerStateEvent.WORKER_STATE_FAILED) {
                                        Dispatcher.platform()
                                            .run(() -> geoTiffEntry.errorLoading(loadingTask.getException()));
                                    }
                                });
                    ...
                    Dispatcher.platform()::runLater);
        } else {
            Dispatcher.platform().runLater(this::syncWithLegacy);
        }
    }
            ...
            elevationModelShiftTypeProperty()
                .addListener(
                    (observable, oldValue, newValue) -> {
                        ...
                        PropertyHelper.setValueSafe(shift, Quantity.of(em.getShift(), Unit.METER));
                    });
            shift.addListener(
                (observable, oldValue, newValue) ->
    ...
    public synchronized void loadWwj() {
        if (startedLoading) {
            return;
        }
    ...
    private void createElevationLayer(Element domElement, AVList params) {
        ...
        Dispatcher.platform().run(() -> GeoTiffEntry.this.type.set(GeoTiffType.ELEVATION));
    }

    private void startImportFile(String displayName, File file, FileStore fileStore) {
