...
class GenericCameraDescription extends NotificationObject implements IMutableGenericCameraDescription {
    ...
    public static class Deserializer implements JsonDeserializer<GenericCameraDescription> {
        ...
        public GenericCameraDescription deserialize(JsonElement arg0, Type arg1, JsonDeserializationContext context)
            ...
            description.setPictureSizeInMB(helper.getDouble("pictureSizeInMB"));
            description.setSdCapacityInGB(helper.getDouble("sdCapacityInGB"));
            description.setEnsoCalibFile(helper.getNullableString("ensoCalibFile"));
            description.setIcarosCalibFile(helper.getNullableString("icarosCalibFile"));
            description.setMenciCalibFile(helper.getNullableString("menciCalibFile"));
            description.setPix4dCalibFile(helper.getNullableString("pix4dCalibFile"));
            description.setAgisoftCalibFile(helper.getNullableString("agisoftCalibFile"));
            ...
            try {
                description.setVideoStreamUri(helper.getString("videoStreamUri"));
            } catch (IllegalArgumentException e) {
                description.setVideoStreamUri(null);
            }
            ...
            array = json.getAsJsonArray("mavlinkParams");
            if (array != null) {
                description.setMavlinkParams(Arrays.asList(context.deserialize(array, MavlinkParam[].class)));
            } else {
    ...
    @Override
    public void setVideoStreamUri(String value) {
        verifyMutable();
        if (value != null && value.isBlank()) {
            value = null;
        }

        if (!java.util.Objects.equals(this.videoStreamUri, value)) {
            String oldValue = this.videoStreamUri;
            this.videoStreamUri = value;
            notifyPropertyChanged(VIDEO_STREAM_URI_PROPERTY, oldValue, value);
        }
    }
    ...
    @Override
    public List<MavlinkParam> getMavlinkParams() {
        return mavlinkParams;
    }

    @Override
    public void setMavlinkParams(List<MavlinkParam> values) {
        Expect.notNull(values, "values");
        verifyMutable();
        if (!this.mavlinkParams.equals(values)) {
    ...
    public boolean equals(Object obj) {
        ...
        return id.equals(other.id)
            ...
            && java.util.Objects.equals(videoStreamUri, other.videoStreamUri)
