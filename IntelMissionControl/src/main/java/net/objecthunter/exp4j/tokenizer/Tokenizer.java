...
public class Tokenizer {
    ...
    public Tokenizer(String expression, final Map<String, Function> userFunctions, final Map<String, Operator> userOperators,
    ...
    public Token nextToken() {
        ...
        if (Character.isDigit(ch) || ch == '.') {
            if (lastToken != null && (lastToken.getType() != Token.TOKEN_OPERATOR && lastToken.getType() != Token.TOKEN_PARENTHESES_OPEN
    ...
    private Token parseFunctionOrVariable() {
        ...
        while (!isEndOfExpression(offset + len - 1) && (Character.isAlphabetic(expression[offset + len - 1])
                || Character.isDigit(expression[offset + len - 1]) || expression[offset + len - 1] == '_')) {
        ...
        if (lastValidToken == null) {
            throw new IllegalArgumentException(
                    "Unable to parse setVariable or function starting at pos " + pos + " in expression '" + new String(expression) + "'");
        }
        pos += lastValidLen;
    ...
    private String parseName() {
        ...
        while (!isEndOfExpression(offset + len) && (Character.isAlphabetic(expression[offset + len])
                || Character.isDigit(expression[offset + len]) || expression[offset + len] == '_')) {
            len++;
        }
        pos += len;
    ...
    private Operator getOperator(String symbol) {
        ...
        if (op == null && symbol.length() == 1) {
            final int argc = (lastToken == null || lastToken.getType() == Token.TOKEN_OPERATOR
                    || lastToken.getType() == Token.TOKEN_PARENTHESES_OPEN) ? 1 : 2;
