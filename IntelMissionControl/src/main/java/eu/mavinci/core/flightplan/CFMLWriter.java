...
public class CFMLWriter {
    ...
    public void writeFlightplan(CFlightplan plan, OutputStream os, String hash) throws IOException {
        ...
        if (plan.getNotes() != null) {
            xml.tag(Tokens.NOTES, Tokens.NOTES, plan.getNotes());
        }
    ...
    private void write(LandingPoint lp, XMLWriter xml) {
        String[] attr = new String[] {Tokens.ID, Tokens.LON, Tokens.LAT, Tokens.MODE, Tokens.ALT_NEW, Tokens.IS_AUTO};
        String[] vals =
            new String[] {
                ...
                lp.getMode().toString(),
                Double.toString(lp.getAltInMAboveFPRefPoint()),
                Boolean.toString(lp.isLandAutomatically())
            };
    ...
    private void write(Point point, XMLWriter xml) {
        ...
        String[] vals =
            new String[] {
                Double.toString(point.getLon()),
                Double.toString(point.getLat()),
                Double.toString(point.getPitch()),
                Double.toString(point.getYaw()),
                Double.toString(point.getAltitude()),
                Boolean.toString(point.isTriggerImage()),
                point.getNote(),
                point.getDistanceSource().toString(),
                Double.toString(point.getGsdMeter()),
                Double.toString(point.getDistanceMeter()),
                Double.toString(point.getFrameDiagonaleMeter()),
                Boolean.toString(point.isTarget())
            };
