...
public class CAirplane extends AAirplaneConnector implements IAirplaneListenerConnectionState, ICAirplane {
    ...
    protected AirportBroadcastListener broadCastListener;
    ...
    protected void init() {
        ...
        broadCastListener = new AirportBroadcastListener();

        CAirport.getInstance().addBackendBroadcastListener(broadCastListener);
    }

    public class AirportBroadcastListener implements IBackendBroadcastListener {

        public void backendListChanged() {
            // System.out.println("backend list cahnged..");
            AirplaneConnectorState conState = getConnectionState();
            if (connector != null) {
                conState = connector.getConnectionState();
            }

            if (conState == AirplaneConnectorState.unconnected) { // if not connected
                // try to find old connection and reconnect, if it was broken manually
                // System.out.println("CAirplane autoconnect"+cache.getLastConnectionLostReason());
                // System.out.println("curConState:" + getConnectionState());
                if (cache.getLastConnectionLostReason() == null
                        || !cache.getLastConnectionLostReason().shouldAutoReconnect()) {
                    return;
                }

                try {
                    String lastSN = getAirplaneCache().getPlaneInfo().serialNumber;
                    // System.out.println("try recon to SN:" + lastSN);
                    Debug.getLog()
                        .log(
                            Level.FINE,
                            "maybe auto reconnect, based on this backend list:"
                                + CAirport.getInstance().getBackendList(),
                            new Exception("just for some stacktrace"));
                    for (BackendState bs : CAirport.getInstance().getBackendList()) {
                        if (!bs.allowReconnect()) {
                            continue;
                        }
                        // System.out.println("next BS:" + bs);
                        for (Port port : bs.getPorts()) {
                            // System.out.println("next SN" + port.info);
                            Debug.getLog().log(Level.FINE, "next port to check:" + port);
                            if (port.info.serialNumber.equals(lastSN)) {
                                // found connection!!
                                // System.out.println("try to recon to" + port);
                                Debug.getLog()
                                    .log(Level.FINE, "Try to reconnect automatically to last connection! TO: " + port);
                                BackToRawConverter btr = new BackToRawConverter(getRootHandler());
                                btr.recv_msg(AirplaneMsgType.INFORMATION.ordinal(), "Try automatical reconnect");

                                // try to reuse connection object
                                CAirplaneTCPconnector con;
                                if (connector instanceof CAirplaneTCPconnector) {
                                    con = (CAirplaneTCPconnector)connector;
                                    con.close();
                                } else {
                                    con =
                                        new CAirplaneTCPconnector(
                                            CAirplane.this,
                                            CAirplaneTCPconnector.DUMMY_CONNECTION_HANDLER,
                                            CAirplaneTCPconnector.DEFAULT_PORT_WAITER);
                                }

                                TCPConnection conDesc = new TCPConnection(bs, port);
                                try {
                                    // System.out.println("try autoconnect");
                                    con.socketConnect(conDesc);
                                    Debug.getLog().log(Level.FINE, "Automatic reconnect succeeded! " + port);
                                } catch (Exception e) {
                                    Debug.getLog().log(Level.FINE, "Automatically Reconnecting Failed " + port, e);
                                }
                                // System.out.println("current connection state is afterwards:" + getConnectionState());
                                return;
                            }
                        }
                    }
        ...
        public void recv_backend(Backend host, MVector<Port> ports) {
            // System.out.println("rcv airplane broadcast");
            try {
                if (host.info.serialNumber.equals(cache.getBackendInfo().serialNumber)) {
                    // System.out.println("deligating broadcast");
                    rootHandler.recv_backend(host, ports);
                }
            } catch (AirplaneCacheEmptyException e) {
            }
        }
    ...
    public void fireGuiClose() {
        ...
        if (broadCastListener != null) {
            CAirport.getInstance().removeBackendBroadcastListener(broadCastListener);
        }
    ...
    public void tryReconnect() {
        if (broadCastListener != null) {
            broadCastListener.backendListChanged();
        }
    }
