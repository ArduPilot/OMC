    ...
    public void startElement(String namespaceURI, String localName, String qName, Attributes atts) throws SAXException {
        ...
        if (qName.equals(ORIGIN)) {
            ...
            if (qName.equals(LANDINGPOINT)) {
                ...
                plan.landingpoint.setModeLegacy(LandingModes.values()[Integer.parseInt(atts.getValue(MODE))]);
                try {
                    String alt = atts.getValue(ALT_INTERNAL);
                    if (alt == null) {
                        alt = atts.getValue(ALT);
                    }

                    plan.landingpoint.setAltitudeLegacy(Integer.parseInt(alt));
                } catch (Throwable t) {
                }

                try {
                    plan.landingpoint.setYawLegacy(Double.parseDouble(atts.getValue(YAW)));
                } catch (Throwable t) {
                }

                try {
                    plan.landingpoint.setIdLegacy(Integer.parseInt(atts.getValue(ID)));
                } catch (Throwable t) {
                    plan.landingpoint.setIdLegacy(ReentryPoint.INVALID_REENTRYPOINT);
                }

                try {
                    plan.landingpoint.setLandingAngleDegLegacy(Double.parseDouble(atts.getValue(LANDING_ANGLE_DEG)));
                } catch (Throwable t) {
                    plan.landingpoint.setLandingAngleDegLegacy(LandingPoint.LANDING_ANGLE_DEG_DEFAULT);
                }

                try {
                    plan.landingpoint.setAltitudeBreakoutLegacy(Integer.parseInt(atts.getValue(ALT_BREAKOUT)));
                } catch (Throwable t) {
                    plan.landingpoint.setAltitudeBreakoutLegacy(LandingPoint.LandingCircleAltitudeBreakoutDef);
                }

                try {
                    plan.landingpoint.setLastAutoLandingRefStartPosLatLegacy(
                        Double.parseDouble(atts.getValue(lastAutoLandingRefStartPosLat)));
                    plan.landingpoint.setLastAutoLandingRefStartPosLonLegacy(
                        Double.parseDouble(atts.getValue(lastAutoLandingRefStartPosLon)));
                    plan.landingpoint.setLastAutoLandingGroundLevelMeterLegacy(
                        Double.parseDouble(atts.getValue(lastAutoLandingGroundLevelMeter)));
                } catch (Throwable t) {
                }
        ...
        if (inBody) {
            if (qName.equals(WAYPOINT)) {
                ...
                bodyToLoad = true;
            } else if (qName.equals(STARTPROCEDURE)) {
                try {
                    CStartProcedure start =
                        FlightplanFactory.getFactory()
                            .newCStartProcedure(Integer.parseInt(atts.getValue(ID)), outerContainer.peek());
                    outerContainer.peek().addToFlightplanContainer(start);
                    try {
                        start.altitude = Integer.parseInt(atts.getValue(ALT));
                        start.hasOwnAltitude = true;
                    } catch (Throwable t) {
                        start.hasOwnAltitude = false;
                    }

                    try {
                        start.isActive = Boolean.parseBoolean(atts.getValue(ACTIVE));
                    } catch (Throwable t) {
                    }
                } catch (Exception e) {
                    throw new SAXException(e);
                ...
                try {
                    outerContainer.peek().addToFlightplanContainer(new Point(outerContainer.peek(), lat, lon));
                } catch (Exception e) {
                    throw new SAXException(e);
                }
            } else if (qName.equals(PREAPPROACH)) {
                try {
                    CPreApproach preApproach =
                        FlightplanFactory.getFactory()
                            .newPreApproach(
                                Double.parseDouble(atts.getValue(LON)),
                                Double.parseDouble(atts.getValue(LAT)),
                                Integer.parseInt(atts.getValue(ALT)),
                                Integer.parseInt(atts.getValue(ID)));
                    outerContainer.peek().addToFlightplanContainer(preApproach);
                } catch (Exception e) {
                    throw new SAXException(e);
                }
            }
