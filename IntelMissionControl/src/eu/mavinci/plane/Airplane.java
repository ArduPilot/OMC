...
public class Airplane extends CAirplane
    ...
    private WindEstimate windEstimate;
    private RtkClient rtkClient;
    private JoystickReader joystickReader = new JoystickReader();
    private IDialogService dialogService;
    private MBackgroundTaskManager backgroundTaskManager;
    ...
    protected void init() {
        ...
        rtkClient = new RtkClient(this);

        backgroundTaskManager = DependencyInjector.getInstance().getInstanceOf(MBackgroundTaskManager.class);
        ftpManager = new FTPManager(this, backgroundTaskManager, dialogService);
        ...
        Dispatcher.schedule(
            new WeakRunnable(tryLoadBackendDumps), Duration.millis(30 * 1000), Duration.millis(2 * 60 * 1000));

        joystickReader.sendDataToPlane(this);
    }
    ...
    private Runnable tryLoadBackendDumps =
        new Runnable() {
            ...
            public void run() {
                ...
                try {
                    Debug.getLog().log(Level.FINE, "try downloading backend dumps");
                    client = getFTPManager().getFTPclient(true, "/");
                    boolean found = false;
                    FTPFile[] list = client.list();
                    // System.out.println("files = "+Arrays.asList(list));
                    for (int i = 0; i != list.length; i++) {
                        final FTPFile ftpFile = list[i];
                        if (ftpFile.getType() == FTPFile.TYPE_FILE) {
                            if (ftpFile.getName().startsWith("core_backend_") && ftpFile.getName().endsWith("zip")) {
                                File target = new File(getFTPFolder(), ftpFile.getName());
                                if (target.exists() && target.length() == ftpFile.getSize()) {
                                    Debug.getLog()
                                        .log(
                                            Level.CONFIG,
                                            "found dump, but allready exists with the same size: " + target);
                                    continue;
                                }

                                Debug.getLog()
                                    .log(Level.CONFIG, "found dump, starting download of: " + ftpFile.getName());
                                // System.out.println("ftpFile:"+ftpFile);
                                client.download(ftpFile.getName(), target);
                                found = true;
                            }
                        }
                    }
                    ...
                    if (found) {
                        Debug.getLog()
                            .log(
                                Level.SEVERE,
                                "Your MAVinci Connector crashed, please provide an automatic error report to MAVinci.");
                    }

                } catch (Exception e) {
                    Debug.getLog().log(Level.CONFIG, "problems downloading backend dumps", e);
                } finally {
                    try {
                        if (client != null) {
    ...
    public synchronized void err_backendConnectionLost(final ConnectionLostReasons reason) {
        ...
        if (reason == ConnectionLostReasons.LOGFILE_AT_END) {
            ...
            backgroundTaskManager.hintJobStatus(languageHelper.getString(KEY + ".connectLostMsg", "" + reason));
        }
    }
    ...
    void checkCamera() {
        ...
        // rtkBaseStationViewCanBeOpen_whenConnectedToLocalSimulation(com.intel.missioncontrol.ui.connection.ConnectionViewTest): (..)
        // joystickViewCanBeOpen_whenConnectedToLocalSimulation(com.intel.missioncontrol.ui.connection.ConnectionViewTest): (..)
    ...
    public String toString() {
        return super.toString() + " sessionFolder:" + baseFolder.get();
    }
    ...
    public void setFlightPhase(AirplaneFlightphase p) {
        if (p.isTakeoffLike()) {
            try {
                if (!CountryDetector.instance.allowProceed(getAirplaneCache().getCurLatLon())) {
                    return;
                }
            } catch (AirplaneCacheEmptyException expected) {
            }

            try {
                if (!CountryDetector.instance.allowProceed(getAirplaneCache().getStartPos())) {
                    return;
                }
            } catch (AirplaneCacheEmptyException expected) {
            }
            ...
            try {
                if (!CountryDetector.instance.allowProceed(getAirplaneCache().getStartPosBaro())) {
                    return;
                }
            } catch (AirplaneCacheEmptyException expected) {
            }
