...
public class AirplaneSim extends AAirplaneConnector implements IAirplaneConnector, Runnable {
    ...
    public static final int MAX_SIM_TIME_OVER_BATTERY_TIME = 20; //also change com.intel.missioncontrol.ui.planning.summary.WarningsView.warning
        ...
        try {
            ...
            while (!simDone) {
                ...
                if (reached2Donce && reachedAlt && reachedCircleCount) {
                    ...
                    if (fpCurObj instanceof Waypoint) {
                    ...
                    } else if (fpCurObj instanceof PreApproach) {
                        PreApproach pre = (PreApproach)fpCurObj;
                        // System.out.println("pre reached: " + curLandingPhase);
                        if (curLandingPhase == 0) { // / phase 0 == undefined
                            assureAlt = AltAssertModes.unasserted;
                            curTargetPos = pre.getPosition();
                            curTargetVec = globe.computePointFromPosition(curTargetPos).transformBy4(transform4);
                            curLandingPhase = 1; // circling down
                            radiusAssert = radiusReached = turnRadius;
                            reached2Donce = true;
                            reachedAlt = false;
                        } else {
                            curLandingPhase = 0; // reset this
                        }
                    } else if (fpCurObj instanceof LandingPoint && !isInCopterMode) {
                        // System.out.println("landing point reached, do something: " + curLandingPhase);
                        ...
                        case DESC_FULL3d:
                            ...
                            break;
                        case DESC_CIRCLE:
                            // System.out.println("curLandingPhase:" + curLandingPhase);
                            if (curLandingPhase == 0) { // / phase 0 == undefined
                                // circling down
                                reached2Donce = true;
                                reachedAlt = false;
                                assureAlt = AltAssertModes.jump;
                                curTargetPos = new Position(landP.getPosition(), landP.getAltBreakoutWithinM());
                                curTargetVec = globe.computePointFromPosition(curTargetPos).transformBy4(transform4);
                                radiusAssert = radiusReached = turnRadius;
                                curLandingPhase = 1;
                            } else if (curLandingPhase == 1) {
                                // waiting on circle for breakout point

                                double yawNorm = yaw.degrees - landP.getYaw();
                                while (yawNorm > 180) {
                                    yawNorm -= 360;
                                }
                                ...
                                // System.out.println("yawNorm:"+yawNorm);
                                if (yawNorm >= 0 && yawNorm <= YAW_RATE_MAX * MAIN_LOOP_STEP * 2) {

                                    // System.out.println("yaw reached: " + vec.z + " " + vec.z );

                                    curLandingPhase = 2;
                                    lastTargetPos =
                                        new Position(landP.getEndOfCircling(null), landP.getAltBreakoutWithinM());
                                    lastTargetVec =
                                        globe.computePointFromPosition(lastTargetPos).transformBy4(transform4);
                                    lastTargetVec2d = new Vec4(lastTargetVec.x, lastTargetVec.y);
                                    Vec4 dVec =
                                        new Vec4(
                                                Math.sin(Math.toRadians(landP.getYaw())),
                                                Math.cos(Math.toRadians(landP.getYaw())))
                                            .multiply3(LandingPoint.lengthLandLineInM / 2);
                                    curTargetVec = lastTargetVec.add3(dVec);
                                    curTargetVec =
                                        new Vec4(curTargetVec.x, curTargetVec.y, LandingPoint.endLandLineAltInM);
                                    curTargetPos =
                                        globe.computePositionFromPoint(curTargetVec.transformBy4(transform4Inv));
                                    reached2Donce = false;
                                    reachedAlt = false;
                                } else {
                                    reached2Donce = true;
                                    reachedAlt = false; // continue circeling???
                                }
                            } else if (curLandingPhase == 2) {
                                // final linear gliding
                                setFlightPhase(AirplaneFlightphase.landing);
                                curLandingPhase = 0; // reset this
                                if (!runAsync) {
                                    simDone = true; // DONE
                        ...
                        default:
                            ...
                            reached2Donce = true;
                            reachedAlt = false; // continue circeling???
                            // System.out.println("set noCirclesCurrentTaget=-1 22");
                            noCirclesCurrentTaget = -1;
                            // just circle for ever ;-)
                        }
                    } else {
                        curLandingPhase = 0; // make sure this is resetted after a restart
                    }
                    ...
                    while (!(fpCurObj instanceof LandingPoint) && curLandingPhase == 0) {
                        ...
                        if (fpCurObj instanceof PreApproach) {
                            PreApproach pre = (PreApproach)fpCurObj;
                            assureAlt = AltAssertModes.unasserted;
                            curTargetPos = new Position(pre.getPosition(), curTargetPos.elevation);
                            curTargetVec = globe.computePointFromPosition(curTargetPos).transformBy4(transform4);
                            radiusAssert = radiusReached = turnRadius;
                            break;
                        }
                        ...
                        if (fpCurObj instanceof LandingPoint) {
                            ...
                            if (isInCopterMode) {
                                ...
                                case DESC_HOLDYAW: // =1 //copters will use this for auto landing on Same as actual
                                    // takeoff location
                                    noCirclesCurrentTaget = -1; // circeling some while
                                    assureAlt = AltAssertModes.unasserted;
                                ...
                                default:
                                    ...
                                    // fallthrough
                                case DESC_STAYAIRBORNE: // =3 //copters will stay airborne on last waypoint, fixedwing
                                ...
                                switch (landP.getMode()) {
                                case DESC_FULL3d:
                                    assureAlt = AltAssertModes.linear;
                                    curTargetPos = landP.getPosition();
                                    radiusAssert = -WP_REACHED_RADIUS + 4;
                                    // System.out.println("HEEEEEEEEEEEEEEEEEEEEEEER: " + radiusAssert);
                                    break;
                                default:
                                    // System.out.println("set circ infinite landingP -> -1");
                                    noCirclesCurrentTaget = -1; // infinite circeling
                                    assureAlt = AltAssertModes.unasserted;
                                    curTargetPos = new Position(landP.getPosition(), curTargetPos.elevation);
                                    break;
                                    // by intention no break here!
                                case DESC_CIRCLE: // this means landing with breakout line, so no infinite circle!?!?
                                    // System.out.println("ladingpoint is next target -> -2");
                                    noCirclesCurrentTaget = -2; // circeling some while
                                    assureAlt = AltAssertModes.unasserted;
                                    curTargetPos = new Position(landP.getPosition(), curTargetPos.elevation);
                                }
                            }
                        ...
                        if (fpCurObj instanceof IFlightplanPositionReferenced) {
                            ...
                            if (posRef instanceof Waypoint) {
                            ...
                            } else if (posRef instanceof StartProcedure) {
                                StartProcedure start = (StartProcedure)posRef;
                                if (!start.isActive()) {
                                    continue;
                                }
                                ...
                                radiusAssert = radiusReached = turnRadius;
                                if (isInCopterMode) {
                                    curTargetPos = new Position(pos, posRef.getAltInMAboveFPRefPoint());
                                    curTargetVec =
                                        globe.computePointFromPosition(curTargetPos).transformBy4(transform4);
                                    noCirclesCurrentTaget = 1; // exactly one circle!
                                    // if (runAsync) System.out.println("will fly start here::" + curTargetPos);
                                } else {
                                    noCirclesCurrentTaget = 1; // exactly one circle!
                                }
                            } else {
    ...
    public synchronized void setFlightPhase(AirplaneFlightphase p) {
        ...
        if (fp != null && p == AirplaneFlightphase.jumpToLanding && p.isFlightphaseOnGround() != 1) {
            setFlightPlan(fp, fp.getLandingpoint().getIdLandingBegin());
            requestFlightPhase();
            return;
        }
