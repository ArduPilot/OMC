...
public class SunCalculator implements IKeepAll {
    public static LatLon subsolarPoint(Calendar time) {
        ...
        // and JD 2451545.0, which is noon 1 January 2000 Universal Time
        {
            ...
            decimalHours =
                time.get(Calendar.HOUR_OF_DAY) + (time.get(Calendar.MINUTE) + time.get(Calendar.SECOND) / 60.0) / 60.0;
            // Calculate current Julian Day
            long aux1 = (time.get(Calendar.MONTH) - 14) / 12;
            long aux2 =
                (1461 * (time.get(Calendar.YEAR) + 4800 + aux1)) / 4
