...
public class CombinedGazetteer implements Gazetteer, IKeepAll {
    ...
    public static CombinedGazetteer createAllGazeteer(WWLayerSettings wwLayerSettings) {
        CombinedGazetteer gz = new CombinedGazetteer();

        String config =DependencyInjector.getInstance()
                .getInstanceOf(ExpertSettings.class)
                .getGeocoderProvider();

        if (config.equalsIgnoreCase(GEOCODER_PROVIDER_GOOGLE)) {
            gz.addSlaveWithCoordinatesGazetter(new GoogleGazetteer(wwLayerSettings));
            gz.addSlaveGazetteer(new GoogleAutoCompleteGazetteer(wwLayerSettings));
        } else if (config.equalsIgnoreCase(GEOCODER_PROVIDER_HEREMAPS)) {
            gz.addSlaveWithCoordinatesGazetter(new HereGazetteer(wwLayerSettings));
            gz.addSlaveGazetteer(new HereAutoCompleteGazetteer(wwLayerSettings));
        } else {
            gz.addSlaveGazetteer(new MapboxAutoCompleteGazetteer(wwLayerSettings));
            gz.addSlaveWithCoordinatesGazetter(new MapboxGazetteer(wwLayerSettings));
        }
            ...
            new Comparator<PointOfInterest>() {
                ...
                public int compare(PointOfInterest o1, PointOfInterest o2) {
                    if (o2.getStringValue(SEARCH_PRIO).equals(o1.getStringValue(SEARCH_PRIO))) {
                        return (
                            o2.getStringValue(AVKey.DISPLAY_NAME) + o2.getLatlon().getLatitude())
                            .compareTo(
                                o1.getStringValue(AVKey.DISPLAY_NAME) + o1.getLatlon().getLatitude());
                    } else {
