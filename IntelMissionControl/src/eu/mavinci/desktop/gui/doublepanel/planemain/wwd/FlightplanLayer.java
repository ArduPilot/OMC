...
import eu.mavinci.flightplan.PreApproach;
...
public class FlightplanLayer extends AbstractLayer
    ...
    private static final Dimension ICON_FLIGHT_POSITION = new Dimension(20, 33);

    public static final Color AOI_COLOR_HOVER_OR_SELECT = new Color(0x00, 0xBB, 0xFF); // 00AEEF
    public static final Color AOI_COLOR_NORMAL = new Color(0xF3, 0xF3, 0xF3); // F3F3F3
    ...
    public FlightplanLayer(
        ...
        textLayer.getTextRenderer().setAlwaysOnTop(false);
        textLayer.getTextRenderer().setEffect(AVKey.TEXT_EFFECT_SHADOW);
        textLayer.setPickEnabled(false);
        textLayerAct.getTextRenderer().setAlwaysOnTop(false);
        textLayerAct.getTextRenderer().setEffect(AVKey.TEXT_EFFECT_SHADOW);
    ...
    private class FlighplanLayerGeneratorVisitor extends AFlightplanVisitor {
        ...
        public boolean visit(IFlightplanRelatedObject fpObj) {
            ...
            if (fpObj instanceof Waypoint) {
            ...
            } else if (fpObj instanceof PreApproach) {
                ...
                PreApproach point = (PreApproach)fpObj;

                // double radius=CLandingPoint.detLandingCircleRadius(plane);
                if (pos == null) {
                    pos = getPositionForWaypoint(point);
                    // lineLayer.addRenderable(new FpCircle(point, pos,radius,true));
                    // lineLayer.addRenderable(new FpCircle(point, pos,radius,false ));
                }

                // if (fpPos.size()>0){
                // Position posHeight = new Position(pos, fpPos.get(fpPos.size()-1).elevation);
                // lineLayer.addRenderable(new FpCircle(point, posHeight,radius,false ));
                // fpPos.add(posHeight);
                // }
                fpPos.add(pos);
                lastIcon = new FpIcon("com/intel/missioncontrol/gfx/point-normal.svg", pos, point, false, false);
                iconLayer.addIcon(lastIcon);
            ...
            } else if (fpObj instanceof StartProcedure) {
                if (operationLevel.get() != OperationLevel.DEBUG) {
                    return false;
                }
                ...
                StartProcedure start = (StartProcedure)fpObj;
                if (isHalfVisible() || LatLon.ZERO.equals(start.getLatLon())) {
                    return false;
                }
                ...
                if (!start.isActive() && fp.getHardwareConfiguration().getPlatformDescription().isInCopterMode()) {
                    return false;
                }

                Position posStartProcedure = posStart;

                if (posStartProcedure == null) {
                    posStartProcedure =
                        new Position(start.getLatLon(), elevationStartPoint + start.getAltInMAboveFPRefPoint());
                } else {
                    posStartProcedure = new Position(posStart, posStart.elevation + start.getAltInMAboveFPRefPoint());
                }

                iconLayer.addIcon(renderStartProcedureIcon(posStartProcedure, start));

                // draw line from ground to alt AND to first waypoint...
                if (posStart == null) { // in case no GPS data is avaliable, assume landing locatio is same as start
                    fpPos.add(new Position(start.getLatLon(), elevationStartPoint));
                    fpPos.add(new Position(start.getLatLon(), elevationStartPoint + start.getAltInMAboveFPRefPoint()));
                } else {
                    fpPos.add(new Position(posStart, posStart.elevation + start.getAltInMAboveFPRefPoint()));
                }

                // no further circles, since they will be drawn anyway on the landingpoint
                ...
                if (!mapView.isFlatEarth()
                    ...
                    sideAttributes.setInteriorOpacity(isShowingAOIfilled ? 0.55 : 0);
                ...
                Position posLanding = new Position(landPoint.getLatLon(), elevationStartPoint);
                if (Position.ZERO.equals(posLanding)) {
                    return false;
                }

                switch (landPoint.getMode()) {
                case DESC_CIRCLE:
                    Position posLandingAlt =
                        new Position(landPoint.getLatLon(), elevationStartPoint + landPoint.getAltInMAboveFPRefPoint());
                    // copters will use this for custom auto landing location
                    if (platformDesc.getAirplaneType().isFixedWing()) {
                        lineLayer.addRenderable(renderYawDirectionLine(landPoint));
                        textLayer.add(renderAltitudeTextLabel(landPoint));
                    }

                    iconLayer.addIcon(renderLandingIcon(posLandingAlt, landPoint));
                    fpPos.add(posLandingAlt);
                    fpPos.add(posLanding);
                    // INTENTIONALLY NO BREAK!!
                case DESC_STAYAIRBORNE:

                    // copters will stay airborne on last waypoint, fixedwing will go to startprocedure location==same
                    // as landing but stay on alt
                    if (platformDesc.isInCopterMode()) return false;

                    // TODO turn radius can change in the config_variables received from the plane -- but no need to
                    // pass the plane anyway
                    double circAlt = elevationStartPoint + landPoint.getAltInMAboveFPRefPoint();
                    double radius = platformDesc.getTurnRadius().convertTo(Unit.METER).getValue().doubleValue();
                    lineLayer.addRenderable(new FpCircleAssertAlt(landPoint, posLanding, radius, true));
                    if (landPoint.getMode() == LandingModes.DESC_CIRCLE) {
                        lineLayer.addRenderable(
                            new FpCircleAssertAlt(
                                landPoint,
                                new Position(posLanding, elevationStartPoint + landPoint.getAltBreakoutWithinM()),
                                radius,
                                false));
                    }

                    lineLayer.addRenderable(
                        new FpCircleAssertAlt(landPoint, new Position(posLanding, circAlt), radius, false));
                    if (fpPos.size() > 0) {
                        Position lastPos = fpPos.get(fpPos.size() - 1);
                        // posListLanding.add(lastPos);
                        ArrayList<Position> landingPositions = new ArrayList<>();
                        Position landingPosition = new Position(landPoint.getLatLon(), lastPos.getElevation());
                        landingPositions.add(landingPosition);
                        fpPos.add(landingPosition);
                        landingPositions.add(new Position(landPoint.getLatLon(), circAlt));
                        if (landPoint.getMode() == LandingModes.DESC_CIRCLE) {
                            landingPositions.add(posLanding);
                        }

                        if (showShadows) {
                            lineLayer.addRenderable(new FpLandingLine(fp, landingPositions, true));
                        }

                        lineLayer.addRenderable(new FpLandingLine(fp, landingPositions, false));
                        makeMarkersFromPosList(landingPositions);
                    }

                    break;

                case DESC_FULL3d:
                    posLanding = new Position(pos, landPoint.getAltBreakoutWithinM() + elevationStartPoint);
                    fpPos.add(posLanding);
                    iconLayer.addIcon(renderLandingIcon(posLanding, landPoint));

                    // highlight landing spot on ground ... outer 20m
                    lineLayer.addRenderable(
                        renderGroundCircle(landPoint, posLanding, new Material(new Color(255, 120, 0)), 20));

                    // inner area with 5m radius
                    lineLayer.addRenderable(
                        renderGroundCircle(landPoint, posLanding, new Material(new Color(95, 255, 0)), 5));

                    // breakup path
                    CWaypoint wpBreakup1 = landPoint.getBreakupWaypoint1();
                    if (wpBreakup1 != null) {
                        ArrayList<Position> posListBreakup = new ArrayList<>();
                        PreApproach preApproach = landPoint.getPreApproach();
                        Ensure.notNull(preApproach, "preApproach");
                        Position posPreApproach = getPositionForWaypoint(preApproach);
                        double ratio =
                            LandingPoint.ALT_SPOTLANDING_POINT_OF_NO_RETURN_METER
                                / (posPreApproach.elevation - posLanding.elevation);
                        Position pointOfNoReturn = Position.interpolate(ratio, posLanding, posPreApproach);
                        Position posBreakup = getPositionForWaypoint(wpBreakup1);
                        Waypoint breakupWaypoint2 = landPoint.getBreakupWaypoint2();
                        Ensure.notNull(breakupWaypoint2, "breakupWaypoint2");
                        Position posBreakup2 = getPositionForWaypoint(breakupWaypoint2);
                        Waypoint downwindWaypoint = landPoint.getDownwindWaypoint();
                        Ensure.notNull(downwindWaypoint, "downwindWaypoint");
                        Position posBeginLanding = getPositionForWaypoint(downwindWaypoint);

                        posListBreakup.add(pointOfNoReturn);
                        posListBreakup.add(posBreakup);
                        posListBreakup.add(posBreakup2);
                        posListBreakup.add(posBeginLanding);

                        lineLayer.addRenderable(new FpLineLoopClosing(landPoint, posListBreakup, false));
                        if (showShadows) {
                            lineLayer.addRenderable(new FpLineLoopClosing(landPoint, posListBreakup, true));
                        }
                    }
                ...
                case DESC_HOLDYAW:
                    ...
                    posLandingAlt = posStart;
                    if (posLandingAlt == null) {
                        posLandingAlt =
                            new Position(
                                landPoint.getLatLon(), elevationStartPoint + landPoint.getAltInMAboveFPRefPoint());
                    } else {
                        posLandingAlt =
                            new Position(posStart, posStart.elevation + landPoint.getAltInMAboveFPRefPoint());
                    }

                    posLanding = new Position(posLandingAlt, elevationStartPoint);

                    // copters will use this for custom auto landing location
                    iconLayerFoot.addIcon(renderLandingIcon(posLandingAlt, landPoint));
                    fpPos.add(posLandingAlt);
                    fpPos.add(posLanding);
                ...
                Position posLand = new Position(point.getLatLon(), -10000);
                iconLayer.addIcon(new FpIconCorner(posLand, point, true));
            } else {
        ...
        private FpLine renderYawDirectionLine(LandingPoint landPoint) {
            ArrayList<Position> posListLanding = new ArrayList<>();
            LatLon begin = landPoint.getEndOfCircling(plane);
            LatLon end =
                LatLon.rhumbEndPosition(
                    begin,
                    Math.toRadians(landPoint.getYaw()),
                    LandingPoint.lengthLandLineInM / elevationModel.getRadiusAt(begin));
            posListLanding.add(new Position(begin, elevationStartPoint + landPoint.getAltBreakoutWithinM()));
            posListLanding.add(new Position(end, elevationStartPoint + LandingPoint.endLandLineAltInM));

            FpLine line = new FpLine(fp, posListLanding, false);
            line.setColor(ColorHelper.removeAlpha(line.getColor()).brighter());
            makeMarkersFromPosList(posListLanding);
            return line;
        }

        private SurfaceFilledCircleWithUserData renderGroundCircle(
                LandingPoint landPoint, Position posLanding, Material material, double radius) {
            SurfaceFilledCircleWithUserData area = new SurfaceFilledCircleWithUserData(landPoint, posLanding, radius);
            ...
            ShapeAttributes atr = new BasicShapeAttributes();
            atr.setDrawInterior(true);
            atr.setInteriorMaterial(material);
            atr.setInteriorOpacity(0.5);
        ...
        private WWIcon renderLandingIcon(Position posLanding, LandingPoint landPoint) {
            ...
            icon.setSize(ICON_FLIGHT_POSITION);
            return icon;
        }

        private WWIcon renderStartProcedureIcon(Position posLanding, IFlightplanRelatedObject fpObject) {
            FpIcon icon =
                new FpIcon("com/intel/missioncontrol/gfx/map_takeoff.svg", posLanding, fpObject, false, false);
            icon.setSize(ICON_FLIGHT_POSITION);
            return icon;
        }

        private UserFacingText renderAltitudeTextLabel(LandingPoint landPoint) {
            Position labelPosition =
                new Position(
                    landPoint.getEndOfCircling(plane), elevationStartPoint + landPoint.getAltBreakoutWithinM());
            OverlayText txt = new OverlayText(Double.toString(landPoint.getAltBreakoutWithinM()), labelPosition);
            txt.setFont(FontHelper.getBaseFont(1));
            txt.setColor(Color.WHITE);
            return txt;
        }
    }
            ...
            if (draggedObject instanceof Waypoint) {
                ...
                if (prev != null) {
                    ...
                    for (IFlightplanStatement tmpStatement : fp) {
                        ...
                        if (tmpStatement instanceof StartProcedure) {
                            StartProcedure start = (StartProcedure)tmpStatement;
                            textLayer.add(getOverlayText(cur, dragPosition, start));
                            break;
                        }
                    }
                }
    ...
    private Position getPositionForWaypoint(IFlightplanPositionReferenced wp) {
        if (wp instanceof LandingPoint) {
            ...
            if (lp.getMode() == LandingModes.DESC_FULL3d) {
                return Position.fromDegrees(wp.getLat(), wp.getLon(), elevationStartPoint + lp.getAltBreakoutWithinM());
            }
        }
    ...
    private class FPcameraPointingLine extends PolylineWithUserData {
        public FPcameraPointingLine(Waypoint point, Position pos, double camTargetAlt, boolean isSelectedThisPoint) {
            ...
            setColor(ColorHelper.setAlpha(FLIGHT_LINE_COLOR, 255));
    ...
    private class FpLandingLine extends FpLineLoopClosing {
        public FpLandingLine(Object userData, Iterable<? extends Position> pos, boolean isShadow) {
            super(userData, pos, isShadow);
            setLineWidth(1.5);
            if (userData == selection) {
                setLineWidth(3 * FlightplanLayer.HIGHLIGHT_SCALE);
            } else {
                setLineWidth(3);
            }
        }
    ...
    void setFont() {
        f = FontHelper.getBaseFont(1.7, Font.BOLD);
    }
    ...
    public void recomputeReady(Recomputer recomputer, boolean anotherRecomputeIsWaiting, long runNo) {
        ...
        // redraw WWD layer
        syncRoot.dispatch(() -> firePropertyChange(AVKey.LAYER, null, FlightplanLayer.this));
    }
}
