...
public class MissionOverviewLayer extends AbstractLayer {
    ...
    private final Map<IMissionInfo, SurfacePolygonWithUserData> polygons = new HashMap<>();
    private final Material missionMaterial;
    private final Material missionSelectedMaterial;
    private final Globe nonFlatEarth;
    private final ReadOnlyAsyncListProperty<IMissionInfo> recentMissions;
    private final List<MissionPreview> missions = new ArrayList<>();
    private IMissionInfo selectedMission;
    ...
    public MissionOverviewLayer(
        ...
        recentMissions.addListener((ListChangeListener<IMissionInfo>)c -> reconstruct(), syncRoot);
        selectionManager
            .currentSelectionProperty()
            .addListener(
                (observable, oldValue, newValue) -> {
                    if (newValue instanceof IMissionInfo) {
                        setSelectedMission((IMissionInfo)newValue);
                    } else {
                        setSelectedMission(null);
                    }
                },
                syncRoot);
        syncRoot.post(this::reconstruct);
    }
    ...
    private void setSelectedMission(IMissionInfo mission) {
    ...
    public List<MissionPreview> getMissions() {
        ...
        missions.clear();
        try (LockedList<IMissionInfo> locked = recentMissions.lock()) {
            for (IMissionInfo missionInfo : locked) {
    ...
    public static class MissionPreview {
        ...
        public final IMissionInfo missionInfo;

        public MissionPreview(String name, Sector sector, IMissionInfo missionInfo) {
            this.name = name;
            this.sector = sector;
