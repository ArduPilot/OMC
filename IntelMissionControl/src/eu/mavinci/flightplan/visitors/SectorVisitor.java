...
public class SectorVisitor extends AFlightplanVisitor implements ISectorReferenced {
    ...
    public boolean visit(IFlightplanRelatedObject fpObj) {
        ...
        if (fpObj instanceof StartProcedure) {
            StartProcedure startProcedure = (StartProcedure)fpObj;
            if (!startProcedure.isActive()) {
                return false;
            }
        }
        ...
        if (fpObj instanceof IPositionReferenced) {
            ...
            OptionalDouble maxElev = ((ISectorReferenced)fpObj).getMaxElev();
            OptionalDouble minElev = ((ISectorReferenced)fpObj).getMinElev();

            if (max.isPresent() && maxElev.isPresent()) {
                max = OptionalDouble.of(Math.max(max.getAsDouble(), maxElev.getAsDouble()));
            } else if (maxElev.isPresent()) {
                max = OptionalDouble.of(maxElev.getAsDouble());
            }

            if (min.isPresent() && minElev.isPresent()) {
                min = OptionalDouble.of(Math.min(min.getAsDouble(), minElev.getAsDouble()));
            } else if (minElev.isPresent()) {
                min = OptionalDouble.of(minElev.getAsDouble());
            }
        }
