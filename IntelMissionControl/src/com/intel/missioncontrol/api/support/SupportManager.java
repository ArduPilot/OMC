...
public class SupportManager implements ISupportManager {
    ...
    public static final String ERROR_DOWNLOADS_FOLDER = "errorDownloads";
    ...
    public SupportManager(
        ...
        Dispatcher.scheduleOnUI(this::scanReportFolder, Duration.seconds(15));
    ...
    private Vector<File> getScreenshots() {
        Vector<File> screenshots = new Vector<File>();
            ...
            () -> {
                for (javafx.stage.Window w : javafx.stage.Window.getWindows()) {
                    ...
                    try {
                        ...
                        LOGGER.warn("could not create screenshot of window ", e1);
                    }
                }
            });
    ...
    private Mission getCurrentOrLastMission() {
        ...
        if (result == null) {
            try (LockedList<IMissionInfo> missionInfos = missionManager.recentMissionInfosProperty().lock()) {
                IMissionInfo info = missionInfos.iterator().next();
                return new Mission(
                    info,
    ...
    public void doDownload(String ticketId, IMProgressMonitor monitor, IApplicationContext applicationContext) {
        ...
        Arrays.stream(file.listFiles())
            .filter(f -> missionManager.isMissionFolder(f))
            .findFirst()
            .ifPresent(
                folder ->
                    applicationContext.loadMissionAsync(
                        new Mission(
                            new MissionInfo(folder.toPath()),
                            mavinciObjectFactory,
                            settingsManager,
                            missionInfoManager,
                            languageHelper,
