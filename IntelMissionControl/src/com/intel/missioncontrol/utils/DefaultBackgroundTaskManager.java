...
@Singleton
public class DefaultBackgroundTaskManager implements IBackgroundTaskManager, MBackgroundTaskManager {
    ...
    @Override
    public void submitTask(String name, FTPManager.Task mTask, long size, Runnable task) {
        ProgressTaskInternal backgroundTask =
            new ProgressTaskInternal(name, size) {
                @Override
                protected Void call() {
                    task.run();
                    return null;
        ...
        AirplaneListenerFileTransferDelegator transferDelegator = new AirplaneListenerFileTransferDelegator();
        transferDelegator.addTransferListener(backgroundTask);
        transferDelegator.addTransferListener(mTask.callback);
        mTask.callback = transferDelegator;
    ...
    @Deprecated
    @Override
    public void hintJobStatus(String status) {
        applicationContext.addToast(Toast.of(ToastType.INFO).setText(status).create());
    }
