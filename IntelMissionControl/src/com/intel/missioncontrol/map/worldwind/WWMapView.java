...
public class WWMapView extends ViewAdapter<MView> implements IWWMapView {
        ...
        super(
            new MView(
                PropertyPath.from(applicationContext.currentMissionProperty())
                    .selectReadOnlyObject(Mission::uavProperty),
                syncRoot));
    ...
    @Override
    public FluentFuture<Void> goToPositionAsync(Position center) {
    ...
    @Override
    public FluentFuture<Void> goToSectorAsync(Sector sector, OptionalDouble maxElev) {
        return syncRoot.dispatch(
            () -> {
                if (getViewMode() != ViewMode.DEFAULT) {
                    return;
