...
public class ElevationModelFactoryEGM extends BasicElevationModelFactory {
    ...
    WWLayerSettings wwLayerSettings;

    public void setWwLayerSettings(WWLayerSettings wwLayerSettings) {
        this.wwLayerSettings = wwLayerSettings;
    }

    @Override
    protected ElevationModel createNonCompoundModel(Element domElement, AVList params, boolean legacy) {
        ElevationModel em;
        if (wwLayerSettings == null) {
            wwLayerSettings = DependencyInjector.getInstance().getInstanceOf(WWLayerSettings.class);
        }
        if (legacy || wwLayerSettings == null) {
            em = super.createNonCompoundModel(domElement, params, legacy);
        } else {
            em = new MapboxElevationModel(domElement, params, wwLayerSettings.mapboxAccessTokenProperty().get());
        }
