        ...
        wmsServerLayers.bindContent(
            ...
            new LifecycleValueConverter<>() {
                ...
                public void remove(WmsServerLayer value) {
                    Dispatcher.post(() -> value.dropCache());
                }
            });
                ...
                c -> {
                    while (c.next()) {
                        for (WmsServer removedWms : c.getRemoved()) {
                            Dispatcher.post(() -> dropCache(removedWms));
                        }
                    }
                });
    ...
    private void refreshWmsCapabilities(WmsServer wmsServer, boolean rescheduleOnFail) {
        strand.post(
            (() -> {
                ...
                try {
                    wmsServer.hasWarningProperty().setAsync(false);
                    wmsServer.warningProperty().setAsync(null);

                    wmsServer.retrieveAndLoadCapabilities();

                } catch (Exception e) {
                    wmsServer.hasWarningProperty().setAsync(true);
                    String error =
                    ...
                    wmsServer.warningProperty().setAsync(warningAndRefresh);
                    ...
                    if (rescheduleOnFail) {
                        ...
                        applicationContext.addToast(
                            Toast.of(ToastType.ALERT).setShowIcon(true).setText(error).setCloseable(true).create());
                    }
                    // TODO refactor
