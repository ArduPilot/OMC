...
public class MavinciObjectFactory {
        ...
        if (flightPlanFile.exists()) {
            try {
                ...
                applicationContext.addToast(
                    Toast.of(ToastType.INFO).setText("Cannot open a flightplan file " + flightPlanFile).create());
            }
        }
    }
    ...
    public CStartProcedure getOrCreateStartProcedure(Flightplan legacyFlightplan) {
        CStartProcedure startProcedure;
        ExtractTypeVisitor<CStartProcedure> startProcedureFilter = new ExtractTypeVisitor<>(CStartProcedure.class);
        startProcedureFilter.startVisit(legacyFlightplan);
        if (startProcedureFilter.filterResults.size() > 0) {
            LOGGER.debug("Found {} start procedure(s)", startProcedureFilter.filterResults.size());
            startProcedure = startProcedureFilter.filterResults.get(0);
        } else {
            startProcedure = FlightplanFactory.getFactory().newCStartProcedure(legacyFlightplan);
            startProcedure.setHasOwnAltitude(true);
            try {
                legacyFlightplan.addToFlightplanContainer(startProcedure);
                LOGGER.debug("Create new CStartProcedure");
            } catch (FlightplanContainerFullException | FlightplanContainerWrongAddingException e) {
                LOGGER.warn("Can not add start procedure to flight plan {}", e.getMessage());
            }
        }
