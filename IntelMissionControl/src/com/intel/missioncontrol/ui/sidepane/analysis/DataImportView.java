...
public class DataImportView extends FancyTabView<DataImportViewModel> {
        ...
        tableView
            ...
            .addListener(
                new ListChangeListener<>() {
                    @Override
                    public void onChanged(Change<? extends FlightLogEntry> change) {
                        Pane header = (Pane)tableView.lookup(".column-header-background");
                        header.heightProperty().removeListener(tableInvalidatedListener); // remove if it's there
                        header.heightProperty().addListener(tableInvalidatedListener);
                        updateTableHeight();
                    }
                });

        addFlightLogsButton.setModel(viewModel.getAddFlightLogsMenuModel());
        addFlightLogsButton.showingProperty().addListener(observable -> viewModel.updateAddFlightLogsMenuModel());
    ...
    private void updateTableHeight() {
        ...
        double height = rows * tableView.getFixedCellSize() + header.getHeight() + 4;
