...
public class AoiEditComponent extends VBox implements Initializable {
    ...
    public AoiEditComponent(AreaOfInterest aoi, Context context, IQuantityStyleProvider quantityStyleProvider, ILanguageHelper languageHelper) {
    ...
    public List<? extends Parent> getEditors() {
        return AoiFactory.getWidgetBuilders(aoi.getType())
            ...
            .filter(f -> (includeFlightDirection) ? (true) : (f != AoiFactory.FLIGHT_DIRECTION_COMPONENT || aoi.getType() == PlanType.WINDMILL))
            .filter(f -> (includeGsd) ? (f != AoiFactory.RADIUS_HEIGHT_HUB_COMPONENT) : (true))  // to keep hub params from side pane
            .filter(f -> (includeGsd) ? (f != AoiFactory.BLADES_COMPONENT) : (true))  // to keep blade params from side pane
