...
import com.intel.missioncontrol.ui.commands.ParameterizedCommandInvocation;
...
import com.intel.missioncontrol.ui.sidepane.planning.landing.StartingLandingView;
import com.intel.missioncontrol.ui.sidepane.planning.landing.StartingLandingViewModel;
...
public class EditFlightplanView extends FancyTabView<EditFlightplanViewModel> {
    ...
    @FXML
    private MenuItem saveFpAndProceedButtonMenuItem;
    ...
    public void initializeView() {
        ...
        saveFpAndProceedButtonMenuItem
            .disableProperty()
            .bind(viewModel.getSaveFlightplanAndProceedCommand().notExecutableProperty());
        saveFpAndProceedButtonMenuItem.setOnAction((a) -> viewModel.getSaveFlightplanAndProceedCommand().execute());
        if (!viewModel.toolsAvailableDebugProperty().get()) {
            saveBtn.getItems().remove(saveFpAndProceedButtonMenuItem);
        }
        ...
        exportButton.visibleProperty().bind(viewModel.showFlightPlanTemplateFooterProperty().not());
        exportButton.managedProperty().bind(viewModel.showFlightPlanTemplateFooterProperty().not());
    ...
    private void loadSectionsNonAoi() {
        ...
        formsContainer.getChildren().add(loadStartingLandingSection());
    ...
    private TitledPane loadStartingLandingSection() {
        ViewTuple<StartingLandingView, StartingLandingViewModel> viewTuple =
            FluentViewLoader.fxmlView(StartingLandingView.class).context(context).load();
        return collapsed(new TitledPane(languageHelper.getString("planningView.startland"), viewTuple.getView()));
    }
