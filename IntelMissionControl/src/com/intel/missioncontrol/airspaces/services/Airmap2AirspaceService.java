...
public class Airmap2AirspaceService implements LocationAwareAirspaceService, SourceAwareAirspaceService {
    ...
    @Inject
    public Airmap2AirspaceService(AirspacesProvidersSettings settings, IPathProvider pathProvider, INetworkInformation networkInformation) {
    ...
    private void configAirmap(AirspacesProvidersSettings settings, IPathProvider pathProvider, INetworkInformation networkInformation) {
        AirMapConfig2 config = new AirMapConfig2();
        config.proxySelector = null;
        config.apiKey = settings.airmapApiKeyProperty().get();
        config.cacheSize = settings.airmapAirspacesPersistentCacheSizeProperty().get() /* in megabytes */ * 1024 * 1024;
        config.networkListener = new OkHttpUtil.NetworkActivityListener() {
            String active = null;
