...
public class Location {
    ...
    public Location(MSpatialReference spatialReference, double x, double y) {
        Pair<Unit<?>, Unit<Length>> units = getUnits(spatialReference);
        this.x = Quantity.of(x, units.first);
        this.y = Quantity.of(y, units.first);
        this.z = Optional.empty();
    }

    public Location(MSpatialReference spatialReference, double x, double y, double z) {
        Pair<Unit<?>, Unit<Length>> units = getUnits(spatialReference);
        this.x = Quantity.of(x, units.first);
        this.y = Quantity.of(y, units.first);
        this.z = Optional.of(Quantity.of(z, units.second));
    }
    ...
    public static Pair<Unit<?>, Unit<Length>> getUnits(MSpatialReference spatialReference) {
        final String xyUnitString = spatialReference.getXyUnit().toLowerCase();
        final String zUnitString = spatialReference.getZUnit().toLowerCase();
        Unit<? extends Quantity<?>>[] xyUnits = Unit.parseSymbol(xyUnitString, Length.class, Angle.class);
        ...
        return new Pair<>(xyUnit, Unit.parseSymbol(zUnitString, Length.class));
    }
}
