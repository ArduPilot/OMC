 ...
 * @author RodgersGB
 * 
 *         modifications made by Brian Wood and Z-Knight modifications made by
 *         Eterna2 (added renderPickerList, etc - for display list without
 *         texture for picking in WWJ) modifications by Christian Doyle: fixed
 *         issue where picking would not work correctly if one had loaded
 *         different models
 */
...
//			System.out.println("loading took: " + (System.currentTimeMillis()-time) + " ms");
			if (this.isDebugging) {
				log.info("Initialized the display list for model: "
						+ model.getSource());
				log.info("loading took: " + (System.currentTimeMillis()-time) + " ms\n");
			}
		}
		...
		if (model.isRenderPicker()) {
			...
			// Disabled lighting for drawing the boundary lines so they are all
			// white (or whatever I chose)
			gl.glDisable(GL2.GL_LIGHTING);
	...
	private int initialize(GL2 gl, Model model) {
		...
		for (int i = 0; i < numMaterials; i++) {
			// TODO:DELETE THIS OLD LINE loadTexture(materials.get(i).strFile,
			// i);
			// TODO:DELETE THIS OLD LINE materials.get(i).texureId = i;

			String subFileName = "";

			// If this is read from a jar file, then try to find the path
			// relative to the model
			int index = file.lastIndexOf('/');
			if (index != -1) {
				subFileName = file.substring(0, index + 1);
			} else {
				// Else, the file path of the model was not from a jar file, so
				// check maybe it
			...
			if (model.getMaterial(i).strFile != null) {
				if (this.isDebugging) {
					log.info("        Material:  " + subFileName
							+ model.getMaterial(i).strFile);
				}

				URL result;
				try {
					result = ResourceRetriever.getResourceAsUrl(subFileName
							+ model.getMaterial(i).strFile);
				} catch (IOException e) {
					if (this.isDebugging) {
						log.warning(" ... failed");
				...
				if (result != null && !result.getPath().endsWith("/")
						&& !result.getPath().endsWith("\\")) {
					loadTexture(gl, result, i);
	 ...
	 * @param id
	 */
	private void loadTexture(GL2 gl,URL url, int id) {
		if (url != null) {
			...
			try {
				...
				log.warning(" ... FAILED loading texture with exception: "
						+ e.getMessage());
				return;
			}

			TextureData td = AWTTextureIO.newTextureData(gl.getGLProfile(), bufferedImage, true);
			texture.put(id, TextureIO.newTexture(td));
		}
	}
	...
	private void genList(GL2 gl, Model model, boolean isFullRender) {
		...
		for (int i = 0; i < model.getNumberOfMeshes(); i++) {
			...
			if (tempObj.hasTexture && texture.get(tempObj.materialID) != null
				...
				if (t == null) {
					throw new NullPointerException("Texture t is null");
				}
				...
				// This is required to repeat textures...because some are not
				// and so only
				// part of the model gets filled in....Might be a way to check
				// if this is
			...
			// sort triangles and polygons of model into separate lists to
			// minimize glBegin/glEnd calls
			...
			// apply first material and save its ID, so it is applied only when
			// changed
			...
			gl.glMaterialfv(GL2.GL_FRONT, GL2.GL_DIFFUSE,
					firstMaterial.diffuseColor.getRGBComponents(rgba), 0);
			gl.glMaterialfv(GL2.GL_FRONT, GL2.GL_AMBIENT,
					firstMaterial.ambientColor.getRGBComponents(rgba), 0);
			gl.glMaterialfv(GL2.GL_FRONT, GL2.GL_SPECULAR,
					firstMaterial.specularColor.getRGBComponents(rgba), 0);
			gl.glMaterialf(GL2.GL_FRONT, GL2.GL_SHININESS,
					firstMaterial.shininess);
			gl.glMaterialfv(GL2.GL_FRONT, GL2.GL_EMISSION,
					firstMaterial.emissive.getRGBComponents(rgba), 0);

			// draw triangles
			gl.glBegin(GL2.GL_TRIANGLES);
			for (int j : triangleIndeces) {
				// If the object has a texture, then do nothing till
				// later...else
				// apply the material property to it.
				if (tempObj.hasTexture && isFullRender) {
					// nothing
					// Has no texture but has a material instead and this
					// material is
					// the FACES material, and not the OBJECTS material ID as
					// being used
					// incorrectly below...by specification, the materialID is
					// associated
					// with a FACE and not an OBJECT
				} else {
					// apply material only when different from current
					if (tempObj.faces[j].materialID != currentMaterialID
							&& tempObj.faces[j].materialID < model
									.getNumberOfMaterials()) {
						currentMaterialID = tempObj.faces[j].materialID;

						Material material = model
								.getMaterial(tempObj.faces[j].materialID);
						gl.glMaterialfv(GL2.GL_FRONT, GL2.GL_DIFFUSE,
								material.diffuseColor.getRGBComponents(rgba), 0);
						gl.glMaterialfv(GL2.GL_FRONT, GL2.GL_AMBIENT,
								material.ambientColor.getRGBComponents(rgba), 0);
						gl.glMaterialfv(GL2.GL_FRONT, GL2.GL_SPECULAR,
						...
						gl.glMaterialf(GL2.GL_FRONT, GL2.GL_SHININESS,
								material.shininess);
						gl.glMaterialfv(GL2.GL_FRONT, GL2.GL_EMISSION,
								material.emissive.getRGBComponents(rgba), 0);
					}
				}
				...
				for (int whichVertex = 0; whichVertex < tempObj.faces[j].vertIndex.length; whichVertex++) {
					...
					try {
						...
						indexType = 0;
						gl.glNormal3f(tempObj.normals[normalIndex].x,
								tempObj.normals[normalIndex].y,
								tempObj.normals[normalIndex].z);
						...
						if (tempObj.hasTexture) {
							if (tempObj.texCoords != null) {
								...
								indexType = 1;
								gl.glTexCoord2f(
										tempObj.texCoords[textureIndex].u,
										tempObj.texCoords[textureIndex].v);
							}
						}
						indexType = 2;
						gl.glVertex3f(tempObj.vertices[vertexIndex].x,
								tempObj.vertices[vertexIndex].y,
								tempObj.vertices[vertexIndex].z);
					} catch (Exception e) {
						e.printStackTrace();
						switch (indexType) {
						case 0:
							log.warning("Normal index " + normalIndex
									+ " is out of bounds");
							break;

						case 1:
							log.warning("Texture index " + textureIndex
									+ " is out of bounds");
							break;

						case 2:
							log.warning("Vertex index " + vertexIndex
									+ " is out of bounds");
							break;
						}
					}
	...
	/**
	 * Render the desired object of the model (specified by an id number of the
	 * object)
	 * 
	 * @param gl
	 * @param id
