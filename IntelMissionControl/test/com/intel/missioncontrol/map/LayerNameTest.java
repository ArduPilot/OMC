...
public class LayerNameTest {
    ...
    @Test
    public void LayerName_DetectKey() {
        LangHelper langHelper = new LangHelper();
        LayerName name;

        name = new LayerName("", "param");
        Assert.assertEquals("", name.toString(langHelper));

        name = new LayerName("%key", "param");
        Assert.assertEquals("value=param", name.toString(langHelper));

        name = new LayerName("%%key", "param");
        Assert.assertEquals("%key", name.toString(langHelper));

        name = new LayerName("key%%notAKey", "param");
        Assert.assertEquals("key%notAKey", name.toString(langHelper));
    }
