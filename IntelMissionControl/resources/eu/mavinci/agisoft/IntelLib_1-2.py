...
def process1(showReady=True):
  try:
    ...
    chunk.accuracy_cameras = [accuracy,accuracy,accuracy]#in this step we have to stay with non zscale, zscale is only important at END of optimization step
    print("Accuracy")
    print(acc)
...
def mavinciOptimize(showReady=True):
  try:
    ...
    if doc.meta['mavinci.gpsType']=='DGPS_RTK' :
      ...
      acc=0.2
      chunk.accuracy_cameras = [acc,acc,acc]#in this step we have to stay with non zscale, zscale is only important at END of optimization step
      correctLevelArm()
      chunk.optimizeCameras(fit_f=True,fit_cxcy=True, fit_aspect=True, fit_skew=True,fit_k1k2k3=True, fit_p1p2=True,fit_k4=True,fit_p3=True,fit_p4=True)
      lastpercentage=0
      ...
      while acc>=0.005:
           ...
           print(s)
           if percentage<0.6:
             print('Stop optimization at '+repr(doc.chunk.accuracy_cameras[0])+'m camera accuracy, since less then 60% would be used.')
             break
           #if percentage<0.90*lastpercentage:
             #print('Stop optimization at '+repr(doc.chunk.accuracy_cameras[0])+'m camera accuracy, since removed amount dropped more then 10%.')
             #break
           ...
           for cam in cams:
             ...
             if err > errCutOff :
               cam.reference.enabled = False
           acc /= 1.5
           doc.chunk.accuracy_cameras = [acc,acc,acc]
           correctLevelArm()
      ...
      doc.chunk.accuracy_cameras = [acc,acc,acc*zScale]#in this step we have to stay with non zscale, zscale is only important at END of optimization step
      correctLevelArm()
      testOp(chunk.optimizeCameras(fit_f=True,fit_cxcy=True, fit_aspect=True, fit_skew=True,fit_k1k2k3=True, fit_p1p2=True,fit_k4=True,fit_p3=True,fit_p4=True),u"optimize cameras")
      chunk.resetRegion()
#     acc = 0.01
