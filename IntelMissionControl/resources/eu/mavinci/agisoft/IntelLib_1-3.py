...
def process1(showReady=True):
  try:
    ...
    if acc != PhotoScan.LowAccuracy:
      ...
      testOp(chunk.alignCameras(),u"[[alignCameras]]")
      saveTimestamp("process1.8")
      chunk.optimizeCameras(fit_f=True,fit_cx=True,fit_cy=True,fit_b1=True,fit_b2=True,fit_k1=True,fit_k2=True,fit_k3=True,fit_k4=True,fit_p1=True,fit_p2=True,fit_p3=True,fit_p4=True,fit_shutter=False)
      doc.save()
    else:
  ...
  try:
    ...
    if doc.meta['mavinci.gpsType']=='DGPS_RTK' :
      ...
      while acc>=0.005:
           ...
           correctLevelArm()
           testOp(chunk.optimizeCameras(fit_f=True,fit_cx=True,fit_cy=True,fit_b1=True,fit_b2=True,fit_k1=True,fit_k2=True,fit_k3=True,fit_k4=True,fit_p1=True,fit_p2=True,fit_p3=True,fit_p4=True,fit_shutter=False),u"optimize cameras")
           chunk.resetRegion()
  ...
  try:
    ...
    if doc.meta['mavinci.dsm.enable']!='0':
      ...
      if blockCountXdem > 1 or blockCountYdem > 1: 
        testOp(chunk.exportDem(os.path.join(folder,'dem.tif'), format = PhotoScan.RasterFormat.RasterFormatTiles, image_format=PhotoScan.ImageFormat.ImageFormatTIFF, write_world=True, projection=crs, dx=d_x_DSM, dy=d_y_DSM, blockw=blockwDEM,blockh=blockhDEM),u"[[exportDem]]")
      else:
        testOp(chunk.exportDem(os.path.join(folder,'dem.tif'), format = PhotoScan.RasterFormat.RasterFormatTiles, image_format=PhotoScan.ImageFormat.ImageFormatTIFF, write_world=True, projection=crs, dx=d_x_DSM, dy=d_y_DSM),u"[[exportDem]]")
