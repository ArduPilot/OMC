# Intel -> Metashape Processing Script
import os
import os.path
import sys
import math
...
APIaccuracy = [Metashape.HighAccuracy,Metashape.MediumAccuracy,Metashape.LowAccuracy]
APIfaceCount = [Metashape.HighFaceCount,Metashape.MediumFaceCount,Metashape.LowFaceCount]
...
def storeLog():
    ...
    filename = os.path.join(folder,time.strftime("Metashape-%Y%m%d-%H%M%S.log"))
    with open(filename, "w") as text_file:
      text_file.write(Metashape.app.console.contents)
...
def openMAVinciDesktop():
  try:
    doc = Metashape.app.document
    if doc ==None:
      Metashape.app.messageBox(u"[[error.noDocChunk]]" )
    ...
    Metashape.app.messageBox(u"[[error]]" )
    return False
...
def checkSetup():
  try:
    ...
    if not Metashape.app.activated:
      Metashape.app.messageBox(u"[[notActivated]]" )
      return False

    doc = Metashape.app.document
    if doc != None and doc.chunk != None and doc.chunk.crs == None:
      Metashape.app.messageBox(u"[[SRSnonSupported]]" % doc.meta['mavinci.wktName'])
    ...
    print(u"computed optimal GPU mask = %s, current GPU mask = %s" % (mask,Metashape.app.gpu_mask))
    
    if Metashape.app.gpu_mask != mask or Metashape.app.cpu_enable != True:
...
def process(showReady=True):
  ...
  try:
      ...
      if showReady:
          Metashape.app.messageBox(u"[[processing.ready]]" % t)
...
def process1(showReady=True):
  try:
    ...
    print("Accuracy")
    print(acc)
    if acc != Metashape.LowAccuracy:
      saveTimestamp("process1.4")
      testOp(chunk.matchPhotos(accuracy=acc, preselection = Metashape.ReferencePreselection,filter_mask=doc.meta['mavinci.useMasks']=='1', keypoint_limit = 40000, tiepoint_limit=4000),u"match photos")
      ...
      testOp(chunk.matchPhotos(accuracy=acc, preselection = Metashape.ReferencePreselection,filter_mask=doc.meta['mavinci.useMasks']=='1', keypoint_limit = 20000, tiepoint_limit=1000),u"match photos")
      saveTimestamp("process1.7")
      testOp(chunk.alignCameras(),u"[[alignCameras]]")
      saveTimestamp("process1.8")
    #saveTimestamp("process1.7")
    ...
    chunk.resetRegion()
    doc.save()
    Metashape.app.document.open(doc.path)#try to fix memory fragmentation by doing this
    doc = Metashape.app.document
    chunk = doc.chunk
    ...
    if showReady:
      Metashape.app.messageBox(u"[[processing1.ready]]" % t)
    return True
  except (KeyboardInterrupt, SystemExit):
    print('>>> INTERRUPTED BY USER <<<')
    return False
...
def correctLevelArm():
    ...
    if chunk.sensors[0].antenna.fixed:
        return False
    vec = Metashape.Vector((float(doc.meta['mavinci.cam.x']),float(doc.meta['mavinci.cam.y']),float(doc.meta['mavinci.cam.z'])))
    mat = Metashape.utils.ypr2mat(chunk.sensors[0].antenna.rotation)
    ...
    if doc.meta['mavinci.gpsType']=='DGPS_RTK' :
      ...
      while acc>=0.005:
           ...
           for cam in cams:
             ...
             location = transform.mulp(Metashape.Vector((0, 0, 0))) + transform.rotation() * Metashape.Matrix.Diag((1, -1, -1)) * cam.sensor.antenna.location
...
#      testOp(chunk.optimizeCameras(fit_f=True,fit_cx=True,fit_cy=True,fit_b1=True,fit_b2=True,fit_k1=True,fit_k2=True,fit_k3=True,fit_k4=True,fit_p1=True,fit_p2=True,fit_p3=True,fit_p4=True),u"optimize cameras")
    ...
    if showReady:
        Metashape.app.messageBox(u"[[mavinciOptimize.ready]]" % t)
    return True
...
def process2(showReady=True):
  try:
    ...
    if doc.meta['mavinci.dense.enable']=='1':
      ...
      print("qualDense")
      print(qualDense)
      testOp(chunk.buildDepthMaps(quality=qualDense,filter=Metashape.FilterMode.MildFiltering),u"[[buildDepthMaps]]")
      ...
      doc = Metashape.app.document
      chunk = doc.chunk
      pointSource=Metashape.DataSource.DenseCloudData
    else:
      pointSource=Metashape.DataSource.PointCloudData
      
    
    #universal export parameters
    crs = Metashape.CoordinateSystem(doc.meta['mavinci.wkt'])
    folder = os.path.abspath(os.path.join(doc.path, os.pardir))
    
    saveTimestamp("process2.15")
    if doc.meta['mavinci.points.enable']!='0':
        testOp(doc.chunk.exportPoints(os.path.join(folder,'points.las'), binary=True, precision=6, colors=True, source = pointSource, format=Metashape.PointsFormat.PointsFormatLAS, projection=crs),u"[[exportPoints]]")
    ...
    #longitude
    v1 = Metashape.Vector( (crd[0],crd[1],0) )
    v2 = Metashape.Vector( (crd[0]+0.01,crd[1],0) )
    ...
    #latitude
    v2 = Metashape.Vector( (crd[0],crd[1]+0.01,0) )
    if doc.chunk.crs == None:
      vm2 = v2
    else:         
      vm2 = doc.chunk.crs.unproject(v2)
    ...
    if doc.meta['mavinci.dsm.enable']!='0':
      ...
      testOp(chunk.buildDem(source=pointSource, interpolation=Metashape.Interpolation.EnabledInterpolation, projection=crs),u"[[buildDem]]")
      ...
      if blockCountXdem > 1 or blockCountYdem > 1: 
        testOp(chunk.exportDem(os.path.join(folder,'dem.tif'), format = Metashape.RasterFormat.RasterFormatTiles, image_format=Metashape.ImageFormat.ImageFormatTIFF, write_world=True, projection=crs, dx=d_x_DSM, dy=d_y_DSM, tiff_big=True, blockw=blockwDEM, blockh=blockhDEM),u"[[exportDem]]")
      else:
        testOp(chunk.exportDem(os.path.join(folder,'dem.tif'), format = Metashape.RasterFormat.RasterFormatTiles, image_format=Metashape.ImageFormat.ImageFormatTIFF, write_world=True, projection=crs, dx=d_x_DSM, dy=d_y_DSM, tiff_big=True),u"[[exportDem]]")
    ...
    if doc.meta['mavinci.ortho.enable']!='0':
      ...
      if color_correction:
        testOp(chunk.calibrateColors(source_data=Metashape.DataSource.ElevationData, color_balance=False),u"[[calibrateColors]]")
      testOp(chunk.buildOrthomosaic(surface=Metashape.DataSource.ElevationData, blending=Metashape.BlendingMode.MosaicBlending, fill_holes=True, projection=crs, dx=d_x, dy=d_y),u"[[buildOrthophoto]]")
      ...
      if blockCountX > 1 or blockCountY > 1:
        testOp(chunk.exportOrthomosaic(os.path.join(folder,'ortho.tif'), format = Metashape.RasterFormat.RasterFormatTiles, image_format=Metashape.ImageFormat.ImageFormatTIFF, raster_transform=Metashape.RasterTransformNone, write_kml=False, write_world=True, projection=crs, dx=d_x, dy=d_y, blockw=blockw,blockh=blockh),u"[[exportOrthophoto]]")
      else:
        testOp(chunk.exportOrthomosaic(os.path.join(folder,'ortho.tif'), format = Metashape.RasterFormat.RasterFormatTiles, image_format=Metashape.ImageFormat.ImageFormatTIFF, raster_transform=Metashape.RasterTransformNone, write_kml=False, write_world=True, projection=crs, dx=d_x, dy=d_y),u"[[exportOrthophoto]]")
    ...
    if showReady:
        Metashape.app.messageBox(u"Intel post GCP processing ready. Completed in %s sec" % t)
    return True
...
class SetupDialog(QtWidgets.QDialog):
    ...
    def gpsTypeChanged(self):
        ...
        self.lblOptimization.setText(u"[[optimizationType.2xMetashape]]" if self.cmbGPStype.currentText()!='DGPS_RTK' else u"[[optimizationType.mavinci]]")
...
def estimateImageQuality():
  try:
    ...
    if not chunk.cameras[-1].meta.__contains__("Image/Quality"):
...
def addJob(jobType = 0):
  ...
  path = doc.path
  if path=='':
    Metashape.app.messageBox(u"[[jobs.saveFirst]]" )
    #print("jobs:",allJobs)
    return
    
  if path in allJobs:
    Metashape.app.messageBox(u"[[jobs.allreadyInList]]" % path)
    return
  
  allJobs.append(path)
  allJobTypes.append(jobType)
  Metashape.app.messageBox(u"[[jobs.added]]" % path)
  #print("jobs:",allJobs)
...
def runJobs():
  ...
  try:
    ...
    for i in range(len(allJobs2)):
      ...
      try:
        Metashape.app.document.open(path)
        doc = Metashape.app.document
        print("-----------------")
    ...
    Metashape.app.messageBox(u"[[batch.ready]]" % t)
    return True
...
def scanDirJobs(jobType = 0):
  try:
    folder = Metashape.app.getExistingDirectory("[[batch.folderScan]]");
    if not folder:
        return False
...
def mavinciInit():
    ...
    print("Intel Plugin: %s" % exportHeader)
    #----------Remaining Init code-------------------
    print("MetashapeVersion: %s"  % (Metashape.app.version))
    if Metashape.app.version.startswith('0.'):
        Metashape.app.messageBox(u"[[notCompatible]]" % Metashape.app.version )
        isCompatible = False
    elif Metashape.app.version.startswith('1.0.'):
        Metashape.app.messageBox(u"[[notCompatible]]" % Metashape.app.version )
        isCompatible = False
    elif Metashape.app.version.startswith('1.1.'):
        Metashape.app.messageBox(u"[[notCompatible]]" % Metashape.app.version )
        isCompatible = False
    elif Metashape.app.version.startswith('1.2.'):
        Metashape.app.messageBox(u"[[notCompatible]]" % Metashape.app.version )
        isCompatible = False    
    elif Metashape.app.version.startswith('1.3.'):
        Metashape.app.messageBox(u"[[notCompatible]]" % Metashape.app.version )
        isCompatible = False
    else:
...
def mavinciMenueInit():
    if mavinciInit():
        ...
        Metashape.app.addMenuItem(label, setup,'CTRL+M')
        
        label = u"[[mavinci]]/[[processing]]"
        Metashape.app.addMenuItem(label, process,'SHIFT+CTRL+M')
        
        label = u"[[mavinci]]/[[processing1]]"
        Metashape.app.addMenuItem(label, process1)
        
        label = u"[[mavinci]]/[[processing2]]"
        Metashape.app.addMenuItem(label, process2)
        
        Metashape.app.addMenuSeparator(u"[[mavinci]]")
        
        #label = u"[[mavinci]]/[[openMAVinciDesktop]]"
        #Metashape.app.addMenuItem(label, openMAVinciDesktop)
        
        label = u"[[mavinci]]/[[estimateImageQuality]]"
        Metashape.app.addMenuItem(label, estimateImageQuality)
        
        label = u"[[mavinci]]/[[optimize]]"
        Metashape.app.addMenuItem(label, mavinciOptimize)
        
        Metashape.app.addMenuSeparator(u"[[mavinci]]")
        
        label = u"[[mavinci]]/[[addJob]]/[[noGCPs]]"
        Metashape.app.addMenuItem(label, addJob)
        label = u"[[mavinci]]/[[addJob]]/[[withGCPsStep1]]"
        Metashape.app.addMenuItem(label, addJob1)
        label = u"[[mavinci]]/[[addJob]]/[[withGCPsStep2]]"
        Metashape.app.addMenuItem(label, addJob2)
        
        label = u"[[mavinci]]/[[addJobScan]]/[[noGCPs]]"
        Metashape.app.addMenuItem(label, scanDirJobs)
        label = u"[[mavinci]]/[[addJobScan]]/[[withGCPsStep1]]"
        Metashape.app.addMenuItem(label, scanDirJobs1)
        label = u"[[mavinci]]/[[addJobScan]]/[[withGCPsStep2]]"
        Metashape.app.addMenuItem(label, scanDirJobs2)
        
        
        label = u"[[mavinci]]/[[showJobs]]"
        Metashape.app.addMenuItem(label, showJobs)
        
        label = u"[[mavinci]]/[[runJobs]]"
        Metashape.app.addMenuItem(label, runJobs)
