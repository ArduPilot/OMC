...
<xsl:param name="heightTolerance" select=".1"/>
...
<xsl:template match="waypoint">
 ...
 <xsl:variable name="changeHeightFirst" select="(../waypoint[1]/@alt div 100.)"/>
   
<xsl:choose>

<xsl:when test="
(number($changeHeightFirst) - number($heightTolerance) >=number(./@alt div 100.) or number(./@alt div 100.)>=number($changeHeightFirst) + $heightTolerance)
and (number($changeHeightP) - number($heightTolerance) >=number(./@alt div 100.) or number(./@alt div 100.)>=number($changeHeightP) + $heightTolerance)
and string(number($changeHeightP)) != 'NaN' ">
<xsl:text>S</xsl:text>;<xsl:value-of select="./@lon" />;<xsl:value-of select="./@lat" />;<xsl:value-of select="./@alt div 100." />;<xsl:value-of select="@camYaw" />;<xsl:value-of select="@camPitch +$pitchOffset" />;<xsl:value-of select="(/flightplan/head/photosettings/@maxGroundSpeedKMH div 3.6)"/>;<xsl:value-of select="./@id"/>, following height: <xsl:value-of select="$changeHeightF"/>, previous height: <xsl:value-of select="$changeHeightP"/>, parent height: <xsl:value-of select="$changeHeightParent"/>, first <xsl:value-of select="$changeHeightFirst"/><xsl:text>
</xsl:text>
</xsl:when>
...
<xsl:text>M</xsl:text>;<xsl:value-of select="./@lon" />;<xsl:value-of select="./@lat" />;<xsl:value-of select="./@alt div 100." />;follow;<xsl:value-of select="@camPitch +$pitchOffset" />;<xsl:value-of select="(/flightplan/head/photosettings/@maxGroundSpeedKMH div 3.6)"/>;<xsl:value-of select="./@id"/>, following height: <xsl:value-of select="$changeHeightF"/>, previous height: <xsl:value-of select="$changeHeightP"/>, parent height: <xsl:value-of select="$changeHeightParent"/><xsl:text>
</xsl:text>
</xsl:when>

<xsl:when test="string(number($changeHeightP)) = 'NaN'
and (number($changeHeightParent) - number($heightTolerance) >=number(./@alt div 100.) or number(./@alt div 100.)>=number($changeHeightParent) + $heightTolerance)
">
<xsl:text>S</xsl:text>;<xsl:value-of select="./@lon" />;<xsl:value-of select="./@lat" />;<xsl:value-of select="./@alt div 100." />;<xsl:value-of select="@camYaw" />;<xsl:value-of select="@camPitch +$pitchOffset" />;<xsl:value-of select="(/flightplan/head/photosettings/@maxGroundSpeedKMH div 3.6)"/>;<xsl:value-of select="./@id"/>, following height: <xsl:value-of select="$changeHeightF"/>, previous height: <xsl:value-of select="$changeHeightP"/>, parent height: <xsl:value-of select="$changeHeightParent"/><xsl:text>
</xsl:text>
</xsl:when>
...
<xsl:when test="
(number($changeHeightP) - number($heightTolerance) >=number(./@alt div 100.) or number(./@alt div 100.)>=number($changeHeightP) + $heightTolerance)
">
