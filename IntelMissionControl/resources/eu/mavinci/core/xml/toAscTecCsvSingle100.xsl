...
<xsl:param name="speed" select="130"/>
...
<xsl:text>S</xsl:text>;<xsl:value-of select="./@lon" />;<xsl:value-of select="./@lat" />;<xsl:value-of select="./@alt div 100." />;<xsl:value-of select="@camYaw" />;<xsl:value-of select="@camPitch +$pitchOffset" />;<xsl:value-of select="($speed div 3.6)"/>;<xsl:value-of select="./@id" />, following height: <xsl:value-of select="$changeHeightF"/>, previous height: <xsl:value-of select="$changeHeightP"/>, parent height: <xsl:value-of select="$changeHeightParent"/><xsl:text>
</xsl:text>
