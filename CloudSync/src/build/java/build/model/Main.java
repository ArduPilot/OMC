...
public class Main {
    ...
    private static final String[] IMPORTS =
        new String[] {
            "com.intel.missioncontrol.geom.*",
            "com.intel.missioncontrol.project.*",
            "com.intel.missioncontrol.project.property.*",
            "com.intel.missioncontrol.project.serialization.*",
            "java.time.Instant",
    ...
    public static void main(String[] args) throws IOException, URISyntaxException {
        ...
        for (int i = 0; i < templateFiles.size(); ++i) {
            ...
            Path filePath = targetDir.resolve(NameHelper.getInterfaceFileName(classes.get(i)));
            try (BufferedWriter writer =
                new BufferedWriter(new OutputStreamWriter(new FileOutputStream(filePath.toFile())))) {
                writer.write(
                    new InterfaceClassGenerator(
                            getPackageName(templatesDir, templateFiles.get(i)), IMPORTS, classes.get(i), classes)
                        .toString());
            }

            filePath = targetDir.resolve(NameHelper.getSnapshotFileName(classes.get(i)));
            try (BufferedWriter writer =
                new BufferedWriter(new OutputStreamWriter(new FileOutputStream(filePath.toFile())))) {
                writer.write(
            ...
            filePath = targetDir.resolve(NameHelper.getModelFileName(classes.get(i)));
            try (BufferedWriter writer =
                new BufferedWriter(new OutputStreamWriter(new FileOutputStream(filePath.toFile())))) {
                writer.write(
