...
final class AsyncListChangeBuilder<E> {
    ...
    private static class SubChange<E> {
        ...
        SubChange(int from, int to, List<E> removed, int[] perm, boolean updated) {
            this.from = from;
            this.to = to;
    ...
    AsyncListChangeBuilder(AsyncObservableListBase<E> list) {
        this.list = list;
    }

    void nextRemove(int idx, E removed) {
        checkState();
        checkAddRemoveList();

        final SubChange<E> last = addRemoveChanges.isEmpty() ? null : addRemoveChanges.get(addRemoveChanges.size() - 1);

        if (last != null && (last.to + last.removed.size()) == idx) {
            last.removed.add(removed);
        } else if (last != null && last.to == idx) {
            last.removed.add(removed);
    ...
    void nextRemove(int idx, List<? extends E> removed) {
        checkState();
    ...
    void nextPermutation(int from, int to, int[] perm) {
        checkState();
    ...
    void nextReplace(int from, int to, List<? extends E> removed) {
        nextRemove(from, removed);
        nextAdd(from, to);
    }

    void nextSet(int idx, E old) {
        nextRemove(idx, old);
        nextAdd(idx, idx + 1);
    }

    void nextUpdate(int idx) {
        checkState();
    ...
    private void commit() {
        ...
        if (changeLock == 0 && (addRemoveNotEmpty || updateNotEmpty || permutationChange != null)) {
            int totalSize =
                (updateChanges != null ? updateChanges.size() : 0)
                    + (addRemoveChanges != null ? addRemoveChanges.size() : 0)
                    + (permutationChange != null ? 1 : 0);
            if (totalSize == 1) {
                if (addRemoveNotEmpty) {
                    list.fireChange(new SingleChange<>(finalizeSubChange(addRemoveChanges.get(0)), list), false);
                    addRemoveChanges.clear();
                } else if (updateNotEmpty) {
                    list.fireChange(new SingleChange<>(finalizeSubChange(updateChanges.get(0)), list), false);
                    updateChanges.clear();
                } else {
                    list.fireChange(new SingleChange<>(finalizeSubChange(permutationChange), list), false);
                ...
                list.fireChange(new IterableChange<>(finalizeSubChangeArray(array), list), false);
    ...
    private SubChange<E>[] finalizeSubChangeArray(final SubChange<E>[] changes) {
    ...
    private SubChange<E> finalizeSubChange(final SubChange<E> c) {
        if (c.perm == null) {
            c.perm = EMPTY_PERM;
        }

        int size = c.to - c.from;
        if (size == 0) {
            c.added = c.updated = Collections.emptyList();
        } else {
            List<E> addedOrUpdatedList = new ArrayList<>(size);
            ListIterator<E> it = list.listIterator(c.from);
            int i = c.from;
            while (i++ < c.to) {
                E element = it.next();
                addedOrUpdatedList.add(element);
            }

            if (c.wasUpdated) {
                c.added = Collections.emptyList();
                c.updated = Collections.unmodifiableList(addedOrUpdatedList);
            } else {
                c.added = Collections.unmodifiableList(addedOrUpdatedList);
                c.updated = Collections.emptyList();
            }
        }
    ...
    static class SingleChange<E> extends AsyncListChangeListener.AsyncChange<E> implements UnsafeListAccess<E> {
        private final SubChange<E> change;
        private boolean onChange;

        SingleChange(SubChange<E> change, AsyncObservableListBase<E> list) {
            super(list);
            this.change = change;
        }
        ...
        public String toString() {
            ...
            if (change.perm.length != 0) {
                ...
                ret = ChangeHelper.addRemoveChangeToString(change.from, change.to, change.added, change.removed);
            }
    ...
    static class IterableChange<E> extends AsyncListChangeListener.AsyncChange<E> implements UnsafeListAccess<E> {

        private SubChange[] changes;
        private int cursor = -1;

        private IterableChange(SubChange[] changes, AsyncObservableList<E> list) {
            super(list);
        ...
        public boolean wasUpdated() {
            checkState();
            return changes[cursor].wasUpdated;
        }
        ...
        public String toString() {
            ...
            while (c < changes.length) {
                ...
                } else if (changes[c].wasUpdated) {
                    b.append(ChangeHelper.updateChangeToString(changes[c].from, changes[c].to));
                } else {
                    b.append(
                        ChangeHelper.addRemoveChangeToString(
                            changes[c].from, changes[c].to, changes[c].added, changes[c].removed));
                }
